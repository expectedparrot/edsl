...
class MessageBuilder:
    ...
    def _build_reasoning_model_content(self) -> str:
        ...
        for file_entry in self.files_list:
            if self._is_pdf_file(file_entry):
                ...
                content_parts.append(self._process_image_for_reasoning_model(file_entry))
            else:
                content_parts.append(self._process_other_file_for_reasoning_model(file_entry))
    ...
    def _build_regular_model_content(self, sync_client=None) -> List[Dict[str, Any]]:
        ...
        for file_entry in self.files_list:
            if self._is_pdf_file(file_entry):
                content.append(self._process_pdf_for_regular_model(file_entry, sync_client))
            else:
    ...
    def _process_pdf_for_regular_model(self, file_entry: "Files", sync_client=None) -> Dict[str, Any]:
