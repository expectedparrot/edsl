    ...
    def fetch_model_info_data(self) -> Dict[str, List[str]]:
        ...
        for source in self.source_preferences:
            if source not in fetchers:
                if self.verbose:
                    print(f"[SOURCE_HANDLER] Fetcher '{source}' not registered. Available: {list(fetchers.keys())}")
                continue
            ...
            try:
                ...
                if len(model_info_fetcher) > 0:
                    if self.verbose:
                        print(f"[SOURCE_HANDLER] Successfully fetched data from source: {source}")
                ...
                if self.verbose:
                    print(f"[SOURCE_HANDLER] Error fetching from source '{source}': {e}")
                continue
