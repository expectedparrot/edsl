import edsl
from reports.charts import ScatterPlotOutput
...
def main():
    ...
    try:
        ...
        for i, q in enumerate(results.survey.questions[:5]):
            print(
                f"{i}: {q.question_name} ({q.question_type}): {q.question_text[:50]}..."
            )
        ...
        if "BarChartOutput" in charts:
            ...
            print("Chart created. Saving as PNG...")
        ...
        if len(numerical_questions) >= 2:
            ...
            print(
                f"Testing scatter plot with questions: {q1.question_name} and {q2.question_name}"
            )
        ...
        if len(numerical_questions) > 0:
            try:
                stats_table = SummaryStatisticsTable(
                    results, numerical_questions[0].question_name
                )
