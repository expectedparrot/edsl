...
class ThemeFinderOutput(ChartOutput):
    ...
    def __init__(self, results, *question_names, free_text_sample_config=None):
        ...
        self._sampled_answers = self._apply_sampling(self.answers, self.question_names[0])
    ...
    def get_theme_counts_chart(self):
        ...
        if self.theme_finder is None:
            return self._create_error_chart(f"ThemeFinder initialization failed: {self._initialization_error}")
    ...
    def get_sentiment_examples_chart(self):
        ...
        try:
            ...
            return self._create_error_chart(f"Sentiment examples chart failed: {str(e)}")
    ...
    def output(self):
        ...
        print_info(f"ThemeFinderOutput.output() called for question: {self.question_names[0]}")
        
        if self.theme_finder is None:
            print_error(f"ThemeFinder initialization failed: {self._initialization_error}")
            error_chart = self._create_error_chart(f"ThemeFinder initialization failed: {self._initialization_error}")
            return error_chart
        ...
        try:
            ...
            if not isinstance(chart, (alt.Chart, alt.LayerChart, alt.FacetChart, alt.TopLevelMixin)):
                print_error(f"Chart is not an Altair Chart, got: {type(chart)}")
                error_chart = self._create_error_chart(f"Invalid chart type: {type(chart)}")
                return error_chart
            ...
            if not isinstance(error_chart, (alt.Chart, alt.LayerChart, alt.FacetChart, alt.TopLevelMixin)):
                ...
                import pandas as pd
                basic_chart = alt.Chart(pd.DataFrame({'x': [1]})).mark_point().encode(x='x:Q')
                return basic_chart
    ...
    def _create_error_chart(self, error_message: str):
        ...
        try:
            ...
            chart = alt.Chart(error_data).mark_text(
                align='center',
                baseline='middle',
                fontSize=12,
                color='red'
            ...
            try:
                ...
                simple_chart = alt.Chart(fallback_data).mark_circle(size=100, color='red').encode(
                ...
                try:
                    minimal_chart = alt.Chart(pd.DataFrame({'a': [1]})).mark_point().encode(x='a')
                    ...
                    raise RuntimeError(f"Unable to create any type of Altair chart: {e3}")
    ...
    def _get_chart_html(self, chart, title):
        ...
        try:
            ...
            return f'<div class="error-message">Error displaying {title}: {str(e)}</div>'
    ...
    def get_all_charts_for_docx(self):
        ...
        try:
            chart = self.get_theme_counts_chart()
            charts.append(("Theme Distribution", chart, "Distribution of themes across responses"))
        except Exception as e:
        ...
        try:
            ...
            charts.append(("Sentiment Analysis by Theme", chart, "Sentiment distribution for each identified theme"))
        except Exception as e:
        ...
        try:
            ...
            charts.append(("Individual Responses by Sentiment", chart, "Each dot represents an individual response, colored by sentiment"))
        except Exception as e:
        ...
        try:
            ...
            charts.append(("Example Quotes by Theme and Sentiment", chart, "Representative quotes for each sentiment level within themes"))
        except Exception as e:
