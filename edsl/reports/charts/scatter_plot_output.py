...
class ScatterPlotOutput(ChartOutput):
    ...
    pretty_short_name = "Scatter plot"
    methodology = "Creates a scatter plot to visualize the relationship between two numerical or linear scale questions"
    ...
    def __init__(self, results, *question_names):
        ...
        self.x_answers = self.results.select(f'answer.{self.question_names[0]}').to_list()
        self.y_answers = self.results.select(f'answer.{self.question_names[1]}').to_list()
    ...
    def can_handle(cls, *question_objs):
        ...
        """
        return (len(question_objs) == 2 and 
                all(q.question_type in ["numerical", "linear_scale"] for q in question_objs))
    ...
    def output(self):
        ...
        df = pd.DataFrame({
            'x_values': self.x_answers,
            'y_values': self.y_answers
        })
        
        # Count frequency of each x,y combination
        df_counts = df.groupby(['x_values', 'y_values']).size().reset_index(name='count')
        
        # Create the scatter plot
        chart = alt.Chart(df_counts).mark_circle().encode(
            x=alt.X('x_values:Q', 
                   title=self.x_question.question_text),
            y=alt.Y('y_values:Q', 
                   title=self.y_question.question_text),
