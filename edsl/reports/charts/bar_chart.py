...
class BarChartOutput(ChartOutput):
    ...
    def __init__(self, results, *question_names):
        ...
        if self.question.question_type == "yes_no":
            ...
            self.question_options = ["Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree"]
    ...
    def can_handle(cls, *question_objs):
        ...
        if len(question_objs) != 1:
            return False
        try:
            return (question_objs[0].question_type in ["multiple_choice", "linear_scale", "yes_no", "likert_five"] and 
                   (hasattr(question_objs[0], 'question_options') or 
                    question_objs[0].question_type in ["yes_no", "likert_five"]))
    ...
    def output(self):
        ...
        if self.question_options:
            for option in self.question_options:
                ...
                if self.question.question_type == "yes_no":
                    count = sum(1 for answer in self.answers if 
                              (option == "Yes" and answer is True) or 
                              (option == "No" and answer is False))
                elif self.question.question_type == "likert_five":
                    ...
                    count = sum(1 for answer in self.answers if likert_map.get(answer) == option)
                else:
        ...
        df = pd.DataFrame({
            ...
            'Percentage': [count / total_responses * 100 if total_responses > 0 else 0 for count in option_counts.values()]
        })
        ...
        if self.question.question_type == "likert_five":
            df['Option'] = pd.Categorical(
                df['Option'],
                categories=self.question_options,
                ordered=True
            )
        ...
        text = bars.mark_text(
            ...
            percentage_label=alt.datum.Percentage + '%'
        )
