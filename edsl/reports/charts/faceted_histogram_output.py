...
class FacetedHistogramOutput(ChartOutput):
    ...
    pretty_name = "Faceted Histogram"
    pretty_short_name = "Faceted histogram"
    methodology = "Creates separate histograms for each category to show how numerical responses vary across different groups"

    def __init__(self, results, *question_names):
        if len(question_names) != 2:
            raise ValueError("FacetedHistogramOutput requires exactly two question names")
