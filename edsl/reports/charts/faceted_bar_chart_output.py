...
class FacetedBarChartOutput(ChartOutput):
    ...
    def __init__(self, results, *question_names):
        if len(question_names) != 2:
            raise ValueError("FacetedBarChartOutput requires exactly two question names")
        super().__init__(results, *question_names)
        
        # Get questions
        self.q1 = self.results.survey.get(self.question_names[0])  # This will be the main bars
        self.q2 = self.results.survey.get(self.question_names[1])  # This will be the facets
    ...
    def can_handle(cls, *question_objs):
        ...
        return (all(q.question_type in ["multiple_choice", "linear_scale"] for q in question_objs) and
    ...
    def output(self):
        ...
        chart = alt.Chart(counts).mark_bar(
            ...
            y=alt.Y('Count:Q',
                   title='Count'),  # Simple generic title
            color=alt.Color('Primary:N',
                          scale=alt.Scale(scheme='category10'),
                          legend=None),  # Remove legend to avoid redundancy
