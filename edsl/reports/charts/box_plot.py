...
class BoxPlotOutput(ChartOutput):
    ...
    def output(self):
        ...
        points = (
            ...
            .transform_window(yc="rank()", groupby=["Category"])
            .transform_calculate(jitter="datum.yc * 0.001")
            .encode(
                x=alt.X("xjitter:Q", scale=alt.Scale(zero=False), axis=None),
                detail="datum.yc",
            )
            .transform_calculate(xjitter="datum.Category + datum.jitter")
        )

        # Create tooltip layer with summary statistics
        tooltips = (
            alt.Chart(stats)
            .mark_rect(opacity=0, width=5)
            .encode(
                x="Category:N",
                tooltip=[
                    alt.Tooltip("Category:N", title=self.cat_question.question_text),
        ...
        final_chart = (
            alt.layer(chart, points, tooltips)
            .properties(
