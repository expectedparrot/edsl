    ...
    def output(self):
        ...
        chart = alt.Chart(df).mark_boxplot(
            ...
            x=alt.X('Category:N',
                   ...
                   sort=list(self.cat_question.question_options)),  # Preserve original order
            y=alt.Y('Value:Q',
        ...
        points = alt.Chart(df).mark_circle(
            ...
            tooltip=[
                ...
                alt.Tooltip('Value:Q', title=self.num_question.question_text, format='.2f')
            ]
        ).transform_window(
            yc='rank()',
            groupby=['Category']
        ).transform_calculate(
            jitter="datum.yc * 0.001"
        ).encode(
            x=alt.X('xjitter:Q',
            ...
            xjitter='datum.Category + datum.jitter'
        )
        
        # Create tooltip layer with summary statistics
        tooltips = alt.Chart(stats).mark_rect(
            opacity=0,
            width=5
        ).encode(
            x='Category:N',
        ...
        # Combine all layers and set properties
        final_chart = alt.layer(chart, points, tooltips).properties(
