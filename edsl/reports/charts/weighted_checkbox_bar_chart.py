...
class WeightedCheckboxBarChart(ChartOutput):
    ...
    def __init__(self, results, *question_names):
        if len(question_names) != 1:
            raise ValueError("WeightedCheckboxBarChart requires exactly one question name")
    ...
    def output(self):
        ...
        chart = alt.Chart(df).mark_bar(
            ...
            x=alt.X('Option:N',
                   ...
                   sort='-y'),  # Sort by height descending
            y=alt.Y('Weighted Count:Q',
                   title='Weighted Number of Responses'),
            color=alt.value('#4C78A8'),  # Use a single, pleasing color
            tooltip=['Option', 
                    alt.Tooltip('Weighted Count:Q', format='.2f')]
        ).properties(
