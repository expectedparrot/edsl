...
class Research:
    ...
    def __init__(
        ...
        *,
        allowed_output_names: list[str] | None = None,
        ...
        for output_class in [ChartOutput, TableOutput]:
            ...
            for output_name, output_class in available_outputs.items():
                if output_class.can_handle(*self.questions):
                    if (
                        self._allowed_output_names is None
                        or output_name in self._allowed_output_names
                    ):
    ...
    def _generate_outputs(self) -> dict[str, type["Output"]]:
        ...
        for name, output_class in self.relevant_outputs.items():
            ...
            if (
                hasattr(output_class, "__init__")
                and "free_text_sample_config"
                in output_class.__init__.__code__.co_varnames
            ):
                generated_outputs[name] = output_class(
                    self.results,
                    *self.question_names,
                ...
                generated_outputs[name] = output_class(
                    self.results, *self.question_names
                )
    ...
    @staticmethod
    def get_possible_output_names(
        results: Results, question_names: list[str]
    ) -> list[str]:
