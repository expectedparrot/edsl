...
from edsl import Scenario, ScenarioList, Survey, Agent
from edsl import QuestionMultipleChoice, QuestionCheckBox, QuestionList
from edsl import Results

from .utilities import create_magazine_quote_layout
from .warning_utils import print_warning, print_info
...
class ThemeFinder: 
    ...
    def __init__(self, answers: List[str], question: str, context: Optional[str] = None): 
        """
        ...
        if not isinstance(self.question, str):
            raise ValueError("Question must be a string---you probably meant to pass the question_text")
        ...
        if all(isinstance(a, (int, float)) or (isinstance(a, str) and a.isdigit()) for a in self.answers if a is not None):
            from .warning_utils import print_info
            print_info("All answers appear to be numeric. ThemeFinder works best with text responses.")
    ...
    def _get_answers_ratings(self) -> Results:
        ...
        q_rating = QuestionLinearScale(question_text = """
        <survey_context>
            {{ scenario.context }}                 
        </survey_context>
    ...
    def _get_themes_results(self, num_themes: int = 7) -> Results: 
        ...
        """
        a = Agent(traits = {"skills": "You are an expert in the field of survey analysis and theme identification."})
        s = Scenario({'context': self.context, 
    ...
    def word_cloud(self) -> None:
        ...
        try:
            ...
            print_warning("wordcloud or matplotlib package not found. Please install with: pip install wordcloud matplotlib")
            return
    ...
    def _get_answer_theme_results(self) -> Results:
        ...
        Returns:
            Results object with theme assignments for each answer
        """
        q_themes = QuestionCheckBox(question_text = """
        <survey_context>
            {{ scenario.context }}                 
        </survey_context>
    ...
    def _get_sentiment_by_theme_results(self) -> Results:
        ...
        """
        scenario = (self
                    .answer_theme_results
                    .select('context', 'question', 'answer', 'relevant_theme')
                    .remove_prefix()
                    .expand('relevant_theme')
                    )

        q_sentiment = QuestionMultipleChoice(question_text = """
        <survey_context>
            {{ scenario.context }}                 
        </survey_context>
        ...
        """,
            question_options = ["Very Negative", "Negative", "Neutral/NA", "Positive", "Very Positive"], 
    ...
    def _get_suggestions(self) -> Results:
        ...
        s = Survey([q_has_suggestion, q_suggestion_text]).add_stop_rule("has_suggestion", "{{ has_suggestion.answer }} == 'No'")
    ...
    def prepare_data(df: pd.DataFrame) -> pd.DataFrame:
        ...
        sentiment_order = ['Very Negative', 'Negative', 'Neutral/NA', 'Positive', 'Very Positive']
        
        # Count occurrences by theme and sentiment
        theme_sentiment_counts = df.groupby(['relevant_theme', 'sentiment']).size().reset_index(name='count')
        
        # Calculate total counts per theme
        theme_totals = theme_sentiment_counts.groupby('relevant_theme')['count'].sum().reset_index(name='total')
        ...
        result['std_error'] = np.sqrt((result['percentage']/100) * (1 - result['percentage']/100) / result['total'])
        ...
        grid = pd.MultiIndex.from_product([all_themes, sentiment_order], 
                                        names=['relevant_theme', 'sentiment']).to_frame(index=False)
        
        # Merge with the existing data
        result = pd.merge(grid, result, on=['relevant_theme', 'sentiment'], how='left')
        
        # Fill NAs with zeros
        result.fillna({'count': 0, 'total': 0, 'percentage': 0, 'std_error': 0, 
                    'ci_lower': 0, 'ci_upper': 0}, inplace=True)
        
        # For rows where total is 0, copy the total from other rows with the same theme
        if (result['total'] == 0).any():
            totals_by_theme = result.loc[result['total'] > 0, ['relevant_theme', 'total']].drop_duplicates()
            result = result.merge(totals_by_theme, on='relevant_theme', how='left', 
                                suffixes=('', '_filled'))
            result.loc[result['total'] == 0, 'total'] = result.loc[result['total'] == 0, 'total_filled']
            result.drop('total_filled', axis=1, inplace=True)
            
        return result

    def create_sentiment_chart(self, facet_column: str = 'relevant_theme', columns: int = 4) -> alt.Chart:
        """
        ...
        """
        raw_df = self.sentiment_by_theme_results.select('relevant_theme', 'sentiment').to_pandas(remove_prefix = True)
        df = self.prepare_data(raw_df)
        # Define the color scheme with colors matching sentiment order
        color_scale = alt.Scale(
            domain=['Very Negative', 'Negative', 'Neutral/NA', 'Positive', 'Very Positive'],
            range=['#8B0000', '#FF4500', '#FFFFFF', '#90EE90', '#006400']
        )
        # Sort themes by total count
        theme_totals = df.groupby('relevant_theme')['count'].sum().sort_values(ascending=False)
        theme_order = theme_totals.index.tolist()
        
        # Create the base chart - bars with black outline
        bars = alt.Chart(df).mark_bar(stroke='black').encode(
            x=alt.X('sentiment:N', 
                    sort=['Very Negative', 'Negative', 'Neutral/NA', 'Positive', 'Very Positive'],
        ...
        chart = (bars + points + error_bars).facet(
            facet=alt.Facet(f'{facet_column}:N', 
                        title=None,
                        sort=theme_order),  # Sort facets by theme order
            columns=columns
        ).resolve_scale(
            y='independent'
        )
        ...
        error_bars = error_bars.properties(
            width=200,
            height=200
        )
    ...
    def create_sentiment_dot_chart(self, columns: int = 4) -> alt.Chart:
        ...
        raw_df = self.sentiment_by_theme_results.select('relevant_theme', 'sentiment', 'answer').to_pandas(remove_prefix=True)
        
        # Create a ranking for each response within theme/sentiment combination to stack dots
        raw_df['y_position'] = raw_df.groupby(['relevant_theme', 'sentiment']).cumcount() + 1
        ...
        theme_totals = raw_df.groupby('relevant_theme').size().sort_values(ascending=False)
        theme_order = theme_totals.index.tolist()
        
        # Create the dot chart
        chart = alt.Chart(raw_df).mark_circle(
            size=60,
            stroke='black',
            strokeWidth=1,
            opacity=0.8
            ...
            x=alt.X('sentiment:N', 
                    sort=['Very Negative', 'Negative', 'Neutral/NA', 'Positive', 'Very Positive'],
                    axis=alt.Axis(labelAngle=45, title='Sentiment')),
            y=alt.Y('y_position:O', 
                    axis=alt.Axis(title='Responses (stacked)', labels=False, ticks=False)),
            ...
            facet=alt.Facet('relevant_theme:N', 
                          title=None,
                          sort=theme_order),
            columns=columns
        ).resolve_scale(
    ...
    def create_theme_counts_chart(self) -> alt.Chart:
        ...
        try:
            raw_df = self.answer_theme_results.select('answer', 'relevant_theme').remove_prefix().expand('relevant_theme').to_pandas()
        except Exception as e:
            print(f"Error in expand operation: {e}")
            print(f"Data structure: {self.answer_theme_results.select('answer', 'relevant_theme').remove_prefix()}")
            raise
        theme_counts = (raw_df
        ...
        chart = alt.Chart(theme_counts).mark_bar(
            stroke='black',
            cornerRadius=2
        ).encode(
            x=alt.X('relevant_theme:N', 
                    sort='-y',
                    title='Theme',
            ...
            width=600,
            height=400,
            title='Response Counts by Theme'
        )
    ...
    def create_sentiment_examples_chart(
        ...
        sentiment_df = self.sentiment_by_theme_results.select('relevant_theme', 'sentiment', 'answer').to_pandas(remove_prefix=True)
    ...
    def create_all_responses_table(self, max_response_length: int = 300) -> pd.DataFrame:
        """
        ...
        raw_df = self.answer_theme_results.select('answer', 'relevant_theme').to_pandas(remove_prefix=True)
        ...
        grouped['Display Response'] = grouped['answer'].apply(
            lambda x: (str(x)[:max_response_length] + '...') if len(str(x)) > max_response_length else str(x)
        )
        
        # Create final DataFrame with selected columns
        result_df = pd.DataFrame({
            'Response': grouped['Display Response'],
    ...
    def create_sentiment_examples_chart_from_data(
        ...
        data = data.copy()
        data = data.groupby(['relevant_theme', 'sentiment']).apply(
        ...
        sentiment_order = ['Very Positive', 'Positive', 'Neutral/NA', 'Negative', 'Very Negative']
        ...
        # Sort themes by total count
        theme_totals = data.groupby('relevant_theme').size().sort_values(ascending=False)
        theme_order = theme_totals.index.tolist()
        ...
        text1 = base.mark_text(
            ...
            alt.datum.row_number <= max_examples
        )
        ...
        chart = (rect + text1 + text2).properties(
            width=400,
            height=300
        ).facet(
            facet=alt.Facet('relevant_theme:N',
    ...
    @classmethod
    def from_data(cls, data, question_name: str, question_text: str, context: Optional[str] = None) -> 'ThemeFinder':
        """
    ...
    def create_sample_sentiment_data() -> pd.DataFrame:
        ...
        data = {
            'relevant_theme': [
                'Work-Life Balance', 'Work-Life Balance', 'Work-Life Balance', 'Work-Life Balance', 'Work-Life Balance',
                'Communication', 'Communication', 'Communication', 'Communication', 'Communication',
                'Technology', 'Technology', 'Technology', 'Technology', 'Technology',
                'Productivity', 'Productivity', 'Productivity', 'Productivity', 'Productivity'
            ],
            'sentiment': [
                'Very Positive', 'Positive', 'Neutral/NA', 'Negative', 'Very Negative',
                'Very Positive', 'Positive', 'Neutral/NA', 'Negative', 'Very Negative',
                'Very Positive', 'Positive', 'Neutral/NA', 'Negative', 'Very Negative',
                'Very Positive', 'Positive', 'Neutral/NA', 'Negative', 'Very Negative'
            ],
    ...
    def report(self) -> str:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(theme_counts_path, 'rb') as f:
                ...
                html_parts.append(f'<img src="data:image/png;base64,{img_base64}" alt="Theme Counts" style="max-width: 100%;">')
            ...
            with open(sentiment_path, 'rb') as f:
                img_base64 = base64.b64encode(f.read()).decode()
                html_parts.append(f'<img src="data:image/png;base64,{img_base64}" alt="Sentiment Analysis" style="max-width: 100%;">')
            
            # Add sentiment dot chart
            html_parts.append("<h2>Individual Responses by Sentiment</h2>")
            html_parts.append("<p>Each dot represents an individual response, colored by sentiment. Hover over dots to see the actual comment.</p>")
            ...
            with open(dot_path, 'rb') as f:
                img_base64 = base64.b64encode(f.read()).decode()
                html_parts.append(f'<img src="data:image/png;base64,{img_base64}" alt="Individual Responses by Sentiment" style="max-width: 100%;">')
            ...
            with open(examples_path, 'rb') as f:
                img_base64 = base64.b64encode(f.read()).decode()
                html_parts.append(f'<img src="data:image/png;base64,{img_base64}" alt="Example Quotes" style="max-width: 100%;">')
        ...
        try:
            ...
            html_parts.append(f'<img src="data:image/png;base64,{img_base64}" alt="Word Cloud" style="max-width: 100%;">')
            
        except ImportError:
            html_parts.append("<p>Note: Word cloud visualization requires wordcloud and matplotlib packages.</p>")
        ...
        if hasattr(self, 'suggestions'):
            ...
            if not suggestions_with_text.empty:
                ...
                html_parts.append("<p>No specific suggestions identified in the responses.</p>")
        ...
        return f"{style}<div class='themefinder-report'>" + "\n".join(html_parts) + "</div>"
