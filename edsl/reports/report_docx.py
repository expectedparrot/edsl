...
class ReportDOCX:
    ...
    def _try_embed_dataframe_or_chart(self, doc, output_obj) -> bool:
        ...
        try:
            ...
            if isinstance(
                generic_output, (alt.Chart, alt.LayerChart, alt.FacetChart)
            ) or isinstance(generic_output, alt.TopLevelMixin):
                ...
                print_info(
                    f"Unrecognized output type from {type(output_obj)}: {type(generic_output)}"
                )
                print_info("Attempting PNG conversion...")
                ...
                print_error(
                    f"Cannot embed output type from {type(output_obj)}: {type(generic_output)}"
                )
                return False

        except Exception as e:
            print_error(
                f"Error in _try_embed_dataframe_or_chart for {type(output_obj)}: {e}"
            )
            return False
    ...
    def _try_convert_to_png_and_embed(self, doc, output_object, source_obj) -> bool:
        ...
        try:
            ...
            if hasattr(output_object, "save") and callable(
                getattr(output_object, "save")
            ):
                try:
            ...
            if (
                not saved
                and hasattr(output_object, "to_png")
                and callable(getattr(output_object, "to_png"))
            ):
                try:
            ...
            if (
                not saved
                and hasattr(output_object, "savefig")
                and callable(getattr(output_object, "savefig"))
            ):
                try:
            ...
            if saved and os.path.exists(tmp_png) and os.path.getsize(tmp_png) > 0:
                try:
                    ...
                    print_success(
                        f"Successfully embedded PNG for {type(source_obj).__name__}"
                    )
                    return True
                except Exception as e:
