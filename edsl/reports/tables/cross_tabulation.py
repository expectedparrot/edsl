...
class CrossTabulationTable(TableOutput):
    ...
    def output(self):
        ...
        # Calculate counts
        counts = pd.crosstab(df["Question1"], df["Question2"], margins=True)
        ...
        # Add row totals
        result["Total (Count)"] = counts["All"]
        result["Total (%)"] = percentages["All"].round(1).astype(str) + "%"
