...
class RegressionTable(TableOutput):
    ...
    pretty_name = "Regression Analysis"
    pretty_short_name = "Regression analysis"
    methodology = "Performs linear regression analysis with the first question as dependent variable and others as independent variables"
    ...
    def __init__(self, results, *question_names):
        ...
        self.ind_vars = [self.results.survey.get(name) for name in self.question_names[1:]]
    ...
    def can_handle(cls, *question_objs):
        ...
        return all(q.question_type in ["numerical", "linear_scale", "multiple_choice"] for q in question_objs[1:])
        ...
        for question in self.ind_vars:
            ...
            if question.question_type == "multiple_choice":
                ...
                for val in unique_vals[:-1]:
                    X_data[f"{question.question_text}: {val}"] = [1 if x == val else 0 for x in answers]
            else:
