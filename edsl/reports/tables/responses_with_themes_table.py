...
class ResponsesWithThemesTable(TableOutput):
    ...
    def __init__(self, results, *question_names, free_text_sample_config=None):
        if len(question_names) != 1:
            raise ValueError("ResponsesWithThemesTable requires exactly one question name")
    ...
    def output(self):
        ...
        responses_df = self.theme_finder.create_all_responses_table(max_response_length=500)
