...
class ChiSquareTable(TableOutput):
    ...
    def output(self):
        ...
        for col in crosstab.columns:
            ...
            formatted_table[f"{col} (%)"] = row_percentages[col].map(
                lambda x: f"{x:.1f}%"
            )
        ...
        stats_table = pd.DataFrame(
            {
                ...
                "Value": [f"{chi2:.3f}", str(dof), f"{p_value:.3f}", f"{len(df):,}"],
            }
        ).set_index("Statistic")
    ...
    def html(self):
        ...
        # Combine the HTML outputs with a header for the statistics
        return (
            f'<div style="margin-bottom: 30px">{crosstab_styled.to_html()}</div>'
            f'<h4 style="margin: 20px 0">Chi-square Test Statistics</h4>'
            f"<div>{stats_styled.to_html()}</div>"
        )
