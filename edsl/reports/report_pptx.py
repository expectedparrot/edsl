...
class ReportPPTX:
    ...
    def _add_title_slide(self, prs):
        ...
        title.text_frame.paragraphs[0].font.color.rgb = RGBColor(37, 99, 235)  # Blue color
    ...
    def _add_overview_slides(self, prs):
        ...
        if self.report.include_respondents_section:
            self._add_content_slide(prs, "Respondent Overview", self.report.respondent_overview)
        
        if self.report.include_scenario_section:
            self._add_content_slide(prs, "Scenario Overview", self.report.scenario_overview)
    ...
    def _add_question_summary_slide(self, prs):
        ...
        if questions_data:
            self._add_table_to_slide(slide, 
    ...
    def _add_question_analysis_slide(self, prs, question_names, output_dict):
        ...
        theme_finder_outputs = [obj for obj in output_dict.values() 
        ...
        if theme_finder_outputs:
            ...
            current_top = self._add_question_metadata(slide, question_names, Inches(1.3))
            ...
            for output_name, output_obj in output_dict.items():
                ...
                # Add output heading
                current_top = self._add_text_box(slide, display_name, 
                                               left=Inches(0.5), top=current_top, 
                                               width=Inches(9), height=Inches(0.5),
                                               font_size=Pt(16), bold=True)
                
                # Add writeup if available
                current_top = self._add_writeup(slide, question_names, output_name, current_top)
    ...
    def _add_theme_section_divider_slide(self, prs, question_names):
        ...
        self._add_text_box(slide, subtitle_text,
                          left=Inches(1), top=Inches(2.5),
                          width=Inches(8), height=Inches(1),
                          font_size=Pt(32), bold=True,
        ...
        self._add_text_box(slide, description_text,
        ...
        self._add_text_box(slide, charts_preview,
                          left=Inches(2), top=Inches(5.8),
                          width=Inches(6), height=Inches(1.5),
                          font_size=Pt(16),
                          alignment=PP_ALIGN.CENTER)
    ...
    def _add_theme_finder_slides(self, prs, question_names, theme_output):
        ...
        try:
            ...
            print_info(f"Creating {len(charts)} separate slides for ThemeFinder charts...")
            ...
            for chart_title, chart, description in charts:
                ...
                if description:
                    current_top = self._add_text_box(slide, description,
                                                   left=Inches(0.5), top=current_top,
                ...
                try:
                    if chart is not None:
                        if self._add_altair_as_image(slide, chart, current_top, width=Inches(9), height=Inches(5.5)):
                            print_success(f"Successfully added slide: {chart_title}")
                        else:
                            print_error(f"Failed to add chart: {chart_title}")
                            self._add_text_box(slide, f"[Could not embed chart: {chart_title}]",
                                             left=Inches(0.5), top=current_top,
                                             width=Inches(9), height=Inches(0.5),
                                             font_size=Pt(14))
                    else:
                        self._add_text_box(slide, f"[Chart not available: {chart_title}]",
                                         left=Inches(0.5), top=current_top,
                                         width=Inches(9), height=Inches(0.5),
                                         font_size=Pt(14))
                except Exception as e:
                    print_error(f"Error adding {chart_title}: {e}")
                    self._add_text_box(slide, f"[Error embedding chart: {chart_title}]\nError: {str(e)}",
                                     left=Inches(0.5), top=current_top,
    ...
    def _add_slide_title(self, slide, title: str):
        ...
        try:
            ...
            if hasattr(slide, 'shapes') and slide.shapes.title:
                ...
                slide.shapes.title.text_frame.paragraphs[0].font.color.rgb = RGBColor(37, 99, 235)
                return
        except Exception:
            pass
    ...
    def _add_text_box(self, slide, text: str, left, top, width, height, 
                      font_size=Pt(14), bold=False, alignment=PP_ALIGN.LEFT):
    ...
    def _add_question_metadata(self, slide, question_names: list, top_position):
        ...
        return self._add_text_box(slide, metadata_text,
                                left=Inches(0.5), top=top_position,
    ...
    def _add_writeup(self, slide, question_names, output_name, current_top):
        ...
        if question_names in self.report.writeups and output_name in self.report.writeups[question_names]:
            ...
            if writeup_enabled:
                ...
                return self._add_text_box(slide, writeup_text,
                                        left=Inches(0.5), top=current_top,
                                        width=Inches(9), height=Inches(1),
                                        font_size=Pt(12))
        return current_top
    ...
    def _add_visualization(self, slide, output_obj, current_top):
        ...
        # Final fallback
        return self._add_text_box(slide, f"[Visualization: {str(output_obj)}]",
                                left=Inches(0.5), top=current_top,
                                width=Inches(9), height=Inches(0.5),
                                font_size=Pt(10))
    ...
    def _try_add_image(self, slide, output_obj, top_position) -> bool:
        ...
        try:
            if hasattr(output_obj, "output"):
                ...
                if isinstance(chart_output, (alt.Chart, alt.LayerChart, alt.FacetChart)) or isinstance(chart_output, alt.TopLevelMixin):
    ...
    def _add_altair_as_image(self, slide, chart, top_position, width=Inches(8), height=Inches(6)) -> bool:
        ...
        try:
            ...
            chart.save(png_path)
            slide.shapes.add_picture(png_path, Inches(1), top_position, width=width, height=height)
        ...
        try:
            if hasattr(output_obj, "output"):
                ...
                if isinstance(output, pd.DataFrame):
                    ...
                    self._add_text_box(slide, df_text,
                                     left=Inches(0.5), top=top_position,
    ...
    def _add_table_to_slide(self, slide, headers: list, data: list, top=Inches(2), left=Inches(0.5)):
        """Add a table to the slide."""
        try:
            rows = len(data) + 1  # +1 for header
            cols = len(headers)
            
            table_shape = slide.shapes.add_table(rows, cols, left, top, Inches(9), Inches(0.5 + rows * 0.3))
        ...
        for question_name in question_names:
            ...
            if question:
                ...
                if hasattr(question, 'question_options') and question.question_options:
                    question_options = ", ".join(str(opt) for opt in question.question_options[:5])  # Limit options
                    if len(question.question_options) > 5:
                        question_options += "..."
                elif hasattr(question, 'option_labels') and question.option_labels:
                    question_options = ", ".join(str(opt) for opt in question.option_labels[:5])
