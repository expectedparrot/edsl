...
def create_survey_bar_chart(
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color('Option:N', scale=alt.Scale(scheme=color_scheme), legend=None),
...
class AnalyzeQuestionMultipleChoice(AnalyzeQuestion):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.question_options is None:
            raise ValueError("Question options must be provided for a multiple choice question")
    ...
    def html_table(self) -> str:
        ...
        option_percentages = {option: (count / total_responses * 100) for option, count in option_counts.items()}
    ...
    def chart(self) -> alt.Chart:
        ...
        if self.test_mode:
            ...
            try:
                ...
                from themes import ThemeFinder
            tf = ThemeFinder(question=self.question.question_text, answers=self.answers, context="Survey results")
...
def create_analyzer(question: Question, answers: List[Any], test_mode: bool = False) -> AnalyzeQuestion:
    """
    ...
    return f(question_text=question.question_text, 
             answers=answers, 
