...
class Analysis(ABC):
    ...
    def __init__(
        self, title: str, description: Optional[str] = None, test_mode: bool = False
    ) -> None:
        """
    ...
    def generate_written_analysis(self) -> str:
        ...
        raise NotImplementedError(
            "Subclasses must implement generate_written_analysis()"
        )
    ...
    def save_chart_as_html(self) -> str:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Generated custom chart HTML with id {chart_id}, length: {len(html)} chars"
                )
                return html
                ...
                html = re.sub(
                    r'getElementById\([\'"]vis[\'"]\)',
                    f'getElementById("{chart_id}")',
                    html,
                )
                html = re.sub(
                    r'vegaEmbed\([\'"]#vis[\'"]\)', f'vegaEmbed("#{chart_id}")', html
                )

                print(
                    f"Generated fallback chart HTML with id {chart_id}, length: {len(html)} chars"
                )
                return html
        except Exception as e:
    ...
    def to_html(self, index: int) -> str:
        ...
        try:
            ...
            print(traceback.format_exc())
            analysis_html = (
                f"<div class='error'>Error generating written analysis: {e}</div>"
            )
...
class FrequencyAnalysis(Analysis):
    ...
    @classmethod
    def from_edsl_question(
        cls, question: Question, answers: List[Any], test_mode: bool = False
    ) -> FrequencyAnalysis:
        """
        ...
        """
        return cls(
            question.question_text, question.question_options, answers, test_mode
        )
    ...
    def generate_chart(self) -> alt.Chart:
        ...
        for option in self.question_options:
            option_counts[option] = sum(
                1 for answer in self.answers if answer == option
            )
        ...
        chart = (
            ...
            .encode(
                x=alt.X(
                    "Option:N",
                    sort=list(self.question_options),
                    title="Response Option",
                ),
                y=alt.Y("Count:Q", title="Number of Responses"),
                color=alt.Color(
                    "Option:N", scale=alt.Scale(scheme="category10"), legend=None
                ),
                tooltip=["Option", "Count"],
            )
            .properties(title="Response Distribution", width=500, height=300)
        )
    ...
    def generate_written_analysis(self) -> str:
        ...
        try:
            ...
            scenario = Scenario(
                {"question_text": self.question.question_text, "chart": file_store}
            )
...
class ThemeAnalysis(Analysis):
    ...
    def __init__(
        ...
        answers: List[Any],
        test_mode: bool = False,
        show_all_output_table: bool = True,
    ) -> None:
        """
    ...
    def theme_finder(self) -> Optional[ThemeFinder]:
        ...
        if self._theme_finder is None and ThemeFinder is not None:
            try:
                ...
                print(
                    f"ThemeFinder initialized successfully for {self.question.question_name}"
                )
            except Exception as e:
    ...
    def generate_chart(self) -> alt.Chart:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Successfully created theme chart for {self.question.question_name}"
                )
                return chart
            except Exception as e:
    ...
    def generate_table(self) -> str:
        ...
        if self.test_mode or self.theme_finder is None:
            ...
            df = pd.DataFrame(
                {
                    ...
                    "Count": sample_counts,
                    "Percentage": [
                        f"{(count / sum(sample_counts) * 100):.1f}%"
            ...
            raw_df = (
                self.theme_finder.answer_theme_results.select(
                    "answer", "relevant_theme"
                )
                .remove_prefix()
            ...
            theme_counts["percentage"] = (
                theme_counts["count"] / total_responses * 100
            ).apply(lambda x: f"{x:.1f}%")

            # Style the DataFrame
            styled_df = theme_counts.style.hide(axis="index").set_table_attributes(
                'class="data-table"'
            )
...
class SentimentAnalysis(Analysis):
    ...
    def __init__(
        self, question: Question, answers: List[Any], test_mode: bool = False
    ) -> None:
        """
    ...
    def generate_chart(self) -> alt.Chart:
        ...
        try:
            print(
                f"Generating sentiment chart for question: {self.question.question_name}"
            )
            ...
            try:
                ...
                print(
                    f"Successfully created sentiment chart for {self.question.question_name}"
                )
                return chart
            except Exception as e:
            ...
            chart = (
                ...
                .properties(width=150, title="Sentiment Analysis (Sample Data)")
            )
    ...
    def generate_table(self) -> str:
        ...
        if not self.test_mode and self.theme_finder is not None:
            try:
                ...
                # Process the data
                sentiment_counts = (
                    raw_df.groupby(["relevant_theme", "sentiment"])
                    .size()
                    .reset_index(name="count")
                )
                ...
                totals = (
                    sentiment_counts.groupby("relevant_theme")["count"]
                    .sum()
                    .reset_index(name="total")
                )
                sentiment_data = pd.merge(sentiment_counts, totals, on="relevant_theme")
                sentiment_data["percentage"] = (
                    sentiment_data["count"] / sentiment_data["total"] * 100
                ).apply(lambda x: f"{x:.1f}%")

                # Create DataFrame
                df = sentiment_data[
                    ["relevant_theme", "sentiment", "count", "percentage"]
                ]
...
class ResponseAnalysis(Analysis):
    ...
    def generate_chart(self) -> alt.Chart:
        ...
        try:
            ...
            if hasattr(
            ...
            ) and self.question.question_type in [
                "multiple_choice",
                "linear_scale",
                "checkbox",
            ]:
                ...
                chart = (
                    ...
                    .properties(title="Response Distribution", width=400, height=400)
                )
            ...
            # For free text, return a bar chart of response lengths
                ...
                if not text_lengths:
                    ...
                    df = pd.DataFrame({"Category": ["No data"], "Value": [0]})
                    chart = (
                        alt.Chart(df)
                        .mark_bar()
                        .encode(x="Category:N", y="Value:Q")
                        .properties(
                            title="No valid responses found", width=500, height=300
                        )
                    )
                    return chart
                ...
                try:
                    ...
                    df = pd.DataFrame(
                        {
                            "Length": [
                                f"{bin_start}-{bin_end}" for bin_start, bin_end in bins
                            ],
                            "Count": bin_counts,
                        }
                    )
                    ...
                    chart = (
                        ...
                        .properties(
                            title="Response Length Distribution", width=500, height=300
                        )
                    )
                    ...
                    chart = (
                        alt.Chart(df)
                        .mark_bar()
                        .encode(x="Statistic:N", y="Value:Q")
                        .properties(title="Response Statistics", width=500, height=300)
                    )
            ...
            df = pd.DataFrame(
                {"Label": ["Response Count"], "Value": [len(self.answers)]}
            )
            ...
            return (
                ...
                .properties(title="Response Count", width=400, height=300)
            )
