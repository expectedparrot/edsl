...
class Cache(Base):
    ...
    @classmethod
    def from_db(cls, cache_id: int) -> Cache:
        """
        Load a Cache from the database using ORM.
        
        Args:
            cache_id: The ID of the cache to load
            
        Returns:
            A Cache object loaded from the database
            
        Raises:
            Exception: If the cache cannot be found or loaded
        """
        ...
        try:
            ...
            # Import load_cache function
            from .orm import load_cache
            
            # Load the cache
            cache = load_cache(session, cache_id)
            
            if not cache:
                raise ValueError(f"No cache found with ID {cache_id}")
                
            return cache
        except Exception as e:
            raise Exception(f"Error loading cache from ORM: {e}")
    ...
    def to_db(self) -> int:
        """
        Save this Cache to the database using ORM.
        
        Returns:
            The ID of the saved cache
            
        Raises:
            Exception: If the cache cannot be saved
        """
        from ..base.db_manager import get_db_manager
        ...
        try:
            ...
            # Import save_cache function
            from .orm import save_cache
            
            # Save the cache
            cache_orm = save_cache(session, self)
            ...
            # Rollback on error
            session.rollback()
            raise Exception(f"Error saving cache to database: {e}")
