...
class RichTableSimpleJobLogger(JobLogger):
    ...
    def __init__(self, verbose=True, **kwargs):
        if not RICH_AVAILABLE:
            raise ImportError(
                "Rich package is required for RichTableSimpleJobLogger. Install with: pip install rich"
            )
    ...
    def _show_progress_url_if_available(self):
        ...
        if (
            not self.progress_url_shown
            and hasattr(self.jobs_info, "progress_bar_url")
            and self.jobs_info.progress_bar_url
        ):
    ...
    def _create_display(self) -> Text:
        ...
        if (
            ...
            total = (
                self.jobs_info.completed_interviews + self.jobs_info.failed_interviews
            )
            if total > 0:
