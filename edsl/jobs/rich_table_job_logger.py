...
class RichTableJobLogger(JobLogger):
    ...
    def __init__(self, verbose=True, **kwargs):
        if not RICH_AVAILABLE:
            raise ImportError("Rich package is required for RichTableJobLogger. Install with: pip install rich")
        ...
        self.status_config = {
            ...
            JobsStatus.COMPLETED: {"icon": "✅", "color": "green", "style": "bold green"},
            JobsStatus.PARTIALLY_FAILED: {"icon": "⚠️", "color": "yellow", "style": "bold yellow"},
    ...
    def _create_job_info_table(self) -> Table:
        ...
        for field, _ in self.jobs_info.__annotations__.items():
            if field in ["pretty_names", "completed_interviews", "failed_interviews", "exception_summary", "model_costs"]:
                continue
    ...
    def _create_status_panel(self, current_status: JobsStatus) -> Panel:
        ...
        content = f"{icon} [{style}]{status_text}[/{style}]{completion_info}{current_msg}"
    ...
    def _create_model_costs_table(self) -> Union[Table, None]:
        ...
        for cost in self.jobs_info.model_costs:
            ...
            row_credits = (cost.input_cost_credits_with_cache or 0) + (cost.output_cost_credits_with_cache or 0)
    ...
    def _create_display(self) -> Layout:
        ...
        footer_content = Panel(
            Align.center("[dim]Use Ctrl+C to stop[/dim]"),
            style="bright_black"
        )
    ...
    def update(self, message: str, status: JobsStatus = JobsStatus.RUNNING):
        ...
        with self.update_lock:
            ...
            self.messages.append(MessageEntry(
                text=message,
                status=status,
