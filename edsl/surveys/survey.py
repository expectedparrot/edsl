...
class Survey(Base):
    ...
    def generate_survey_from_topic(
        ...
        if scenario_keys:
            scenario_vars = ", ".join([f"{{{{ scenario.{key} }}}}" for key in scenario_keys])
...
- Question names should be short, unique references (like "satisfaction", "age", "preference"){scenario_instruction}
        ...
        if not items:
            ...
            for i in range(n_questions):
                ...
                if scenario_keys:
                    ...
                    question_text = f"What are your thoughts on {topic}? (Question {i+1})"
                
                # Use pattern-based type inference for fallback questions
                text_lower = question_text.lower()
                if any(word in text_lower for word in ['satisfied', 'satisfaction', 'happy', 'pleased', 'likely', 'probability', 'chance', 'often', 'frequency', 'agree', 'disagree', 'opinion']):
                    question_obj = QuestionMultipleChoice(
                        question_name=f"q{i}",
                        question_text=question_text,
                        question_options=["Very satisfied", "Satisfied", "Neutral", "Dissatisfied", "Very dissatisfied"]
                    )
                elif any(word in text_lower for word in ['features', 'functionality', 'capabilities', 'value', 'like', 'benefits', 'advantages', 'perks', 'important', 'channels', 'methods', 'ways', 'prefer', 'contact']):
                    question_obj = QuestionCheckBox(
                        question_name=f"q{i}",
                        question_text=question_text,
                        question_options=["User interface", "Performance", "Security", "Customer support", "Pricing"]
                    )
                elif any(word in text_lower for word in ['rate', 'rating', 'scale', 'level', 'score']):
            ...
            for i, item in enumerate(items):
                ...
                if scenario_keys and "question_text" in item:
                    ...
                    has_scenario_vars = any(f"{{{{ scenario.{key} }}}}" in original_text for key in scenario_keys)
                    if not has_scenario_vars:
                        # Add scenario variables to the question text
                        scenario_vars = " ".join([f"{{{{ scenario.{key} }}}}" for key in scenario_keys])
                        item["question_text"] = f"{original_text} (Context: {scenario_vars})"
    ...
    def generate_survey_from_questions(
        ...
        for i, text in enumerate(question_texts):
            ...
            if scenario_keys:
                ...
                scenario_vars = ", ".join([f"{{{{ scenario.{key} }}}}" for key in scenario_keys])
                enhanced_text = f"{text} (Use these variables where appropriate: {scenario_vars})"
            else:
                enhanced_text = text
        ...
        if question_types is None or any(not qt for qt in question_types):
            ...
            for data in question_data:
                if "question_type" not in data or not data["question_type"]:
                    ...
                    if any(word in text for word in ['satisfied', 'satisfaction', 'happy', 'pleased', 'likely', 'probability', 'chance', 'often', 'frequency', 'agree', 'disagree', 'opinion']):
                        data["question_type"] = "MultipleChoice"
                    elif any(word in text for word in ['features', 'functionality', 'capabilities', 'value', 'like', 'benefits', 'advantages', 'perks', 'important', 'channels', 'methods', 'ways', 'prefer', 'contact']):
                        data["question_type"] = "CheckBox"
                    elif any(word in text for word in ['rate', 'rating', 'scale', 'level', 'score']):
        ...
        for i, data in enumerate(question_data):
            ...
            if scenario_keys and "question_text" in data:
                ...
                if not has_scenario_vars:
                    # Add scenario variables to the question text
                    scenario_vars = " ".join([f"{{{{ scenario.{key} }}}}" for key in scenario_keys])
                    data["question_text"] = f"{original_text} (Context: {scenario_vars})"
    ...
    @classmethod
    def _infer_question_types(cls, question_data: List[Dict[str, Any]], model: "LanguageModel") -> List[Dict[str, Any]]:
...
- Return ONLY valid JSON (a list). No commentary.
        ...
        for i, row in enumerate(out):
            normalized_row = {
                "question_text": row.get("question_text", question_data[i]["question_text"]).strip(),
                "question_type": row.get("question_type", "FreeText").strip(),
                "question_name": row.get("question_name") or question_data[i].get("question_name", f"q{i}"),
    ...
    @classmethod
    def _create_question_from_dict(cls, data: Dict[str, Any], default_name: str) -> "QuestionBase":
        """Create a question object from a dictionary."""
        from ..questions import (
            QuestionFreeText, QuestionMultipleChoice, QuestionLinearScale, QuestionCheckBox
        )
        import re
        import uuid
        ...
        def _slugify(text: str, fallback_len: int = 8) -> str:
            ...
            question_words = {
                ...
                'do', 'are', 'would', 'have', 'did', 'will', 'can', 'should', 'could',
                'is', 'was', 'were', 'does', 'you', 'your', 'the', 'a', 'an'
            }
        ...
        if qtype in ("multiplechoice", "multiple_choice", "mc", "single_select"):
            ...
            if not opts:
                ...
                if any(word in text.lower() for word in ['satisfied', 'satisfaction', 'happy', 'pleased']):
                    opts = ["Very satisfied", "Satisfied", "Neutral", "Dissatisfied", "Very dissatisfied"]
                elif any(word in text.lower() for word in ['likely', 'probability', 'chance']):
                    opts = ["Very likely", "Likely", "Neutral", "Unlikely", "Very unlikely"]
                elif any(word in text.lower() for word in ['often', 'frequency', 'regularly']):
                    opts = ["Very often", "Often", "Sometimes", "Rarely", "Never"]
                elif any(word in text.lower() for word in ['agree', 'disagree', 'opinion']):
        ...
        if qtype in ("checkbox", "check_box", "multi_select", "multiselect"):
            ...
            if not opts:
                # Generate more contextual options based on question text
                if any(word in text.lower() for word in ['features', 'functionality', 'capabilities', 'value', 'like']):
                    opts = ["User interface", "Performance", "Security", "Customer support", "Pricing"]
                elif any(word in text.lower() for word in ['benefits', 'advantages', 'perks', 'important']):
                    opts = ["Health insurance", "Retirement plan", "Remote work", "Paid time off", "Professional development"]
                elif any(word in text.lower() for word in ['channels', 'methods', 'ways', 'prefer', 'contact']):
