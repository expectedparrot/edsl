...
class OptionOrganizationProcessor(BaseProcessor):
    ...
    async def process(
        ...
        organized_options = self._organize_options(
            original_options, question.question_text
        )
    ...
    def _organize_options(
        self, options: List[Union[str, int, float]], question_text: str
    ...
    def _looks_like_numeric_ranges(self, options: List[Union[str, int, float]]) -> bool:
        ...
        for opt in options:
            if isinstance(opt, str):
                ...
                if re.search(
                    r"\d+[-–]\d+|\d+\s*(employee|person|year|dollar)",
                    opt,
                    re.IGNORECASE,
                ):
                    range_count += 1
                # Also count single numbers with units
                    ...
                    r"^\d+\s*(employee|person|year|dollar)", opt, re.IGNORECASE
                ):
                    range_count += 1
    ...
    def _sort_numeric_ranges(
        self, options: List[Union[str, int, float]]
    ) -> List[Union[str, int, float]]:
        ...
        def extract_range_start(option: Union[str, int, float]) -> float:
            ...
            if re.search(
                r"(don\'t know|unknown|other|none)", option_str, re.IGNORECASE
            ):
            ...
            if re.search(r"(more than|over|\+)", option_str, re.IGNORECASE):
                ...
                if match:
                    return (
                        float(match.group(1).replace(",", "")) + 0.1
    ...
    def _looks_like_frequency_scale(
        self, options: List[Union[str, int, float]]
    ) -> bool:
