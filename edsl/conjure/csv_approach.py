...
def close_and_start_group(current_group, groups, index, new_data_type, next_start_offset=0):
...
def is_continuation_of_previous(index: int, first_lines: list[str], second_lines: list[str]) -> bool:
    """
    ...
    for index, column in enumerate(columns):
        ...
        if col_type == ColumnType.PREPEND:
            ...
            current_group = close_and_start_group(
                ...
                DataType.PREPEND, next_start_offset=1
            )
        elif col_type in (ColumnType.QUESTION_START, ColumnType.QUESTION_VIEWED):
            current_group = close_and_start_group(
                current_group, groups, index,
                DataType.SURVEY_RESPONSE, next_start_offset=0
            )
    ...
    for group in groups:
        ...
        if len(group) == 2:
            ...
            if first_second == "Question Viewed" and second_second == "Response":
                ...
                mappings.append(QuestionMapping(
...
def build_response_records(columns, question_mappings: List[QuestionMapping]) -> List[dict]:
    """
...
question_mappings = build_question_mappings(groups, ImportedSurvey.columns, second_lines)
...
if monadic_questions and len(sl) > 0:
    jobs = survey.by(al).by(sl).include_when("{{ scenario._index == agent.traits['_scenario_index'] }}")
