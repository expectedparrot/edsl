...
class ImportSurveyMonkey:
    ...
    def _close_and_start_group(
        self,
        current_group: GroupData,
        index: int,
        new_data_type: DataType,
        next_start_offset: int = 0,
    ) -> GroupData:
    ...
    def _build_groups(self):
        ...
        for index, column in enumerate(self._columns):
            ...
            if col_type == ColumnType.PREPEND:
                self._prepend_data.append(
                    PrependData(index, self._first_lines[index], list(column[2:]))
                )
    ...
    def _tally_responses(self, column_index: int) -> List[str]:
        ...
        if self.repair_excel_dates and self._excel_repairer and options:
            ...
            try:
                repair_result = self._excel_repairer.repair_question_options(
                    question_id, options
                )
                if repair_result.any_repairs_applied:
                    self._log(
                        f"Applied {len(repair_result.repairs_made)} repairs to options in column {column_index}"
                    )
                ...
                self._log(
                    f"Warning: Excel date repair failed for column {column_index}: {e}"
                )

        return options

    def _apply_semantic_ordering(
        ...
        """
        if (
            not self.order_options_semantically
            or not self._option_orderer
            or not options
            or len(options) < 2
        ):
            return options
        ...
        try:
            ...
            if ordering_result.ordering_details.reordering_applied:
                self._log(
                    f"Semantically reordered options for {question_identifier}: "
            ...
            self._log(
                f"Warning: Semantic ordering failed for {question_identifier}: {e}"
            )
            return options
    ...
    def _build_survey(self):
        ...
        for group in self._groups:
            ...
            if monadic:
                self._log(
                    f"Detected monadic question with {len(monadic.slots)} slot(s)"
                )
            ...
            if len(group) == 1:
                ...
                if second_line == "Response":
                    ...
                    ordered_options = self._apply_semantic_ordering(
                        question_text, question_name, raw_options
                    )
                ...
                if options == ["Response", "Other (please specify)"]:
                    ...
                    if self.repair_excel_dates and self._excel_repairer and options:
                        ...
                        try:
                            repair_result = (
                                self._excel_repairer.repair_question_options(
                                    question_id, options
                                )
                            )
                            if repair_result.any_repairs_applied:
                                self._log(
                                    f"Applied {len(repair_result.repairs_made)} repairs to checkbox options for {question_name}"
                                )

                                # Update repair mapping for response value translation
                                for repair in repair_result.repairs_made:
                                    self._repair_mapping[repair.original] = (
                                        repair.repaired
                                    )
                            ...
                            self._log(
                                f"Warning: Excel date repair failed for checkbox question {question_name}: {e}"
                            )

                    # Apply semantic ordering to checkbox options
                    ordered_options = self._apply_semantic_ordering(
                        question_text, f"{question_name}_checkbox", repaired_options
                    )
            ...
            if text and text.strip():
                ...
                if slot.position < len(tokens):
                    ...
                    if value in value_to_scenario_idx:
                        self._agent_to_scenario_idx[agent_idx] = value_to_scenario_idx[
                            value
                        ]
        ...
        for row_idx in range(num_rows):
            ...
            for mapping in self._question_mappings:
                if mapping.is_checkbox:
                    ...
                    primary_answer = self._columns[mapping.column_indices[0]][
                        data_row_idx
                    ]
                    other_text = ""
                    if len(mapping.column_indices) > 1:
                        other_text = self._columns[mapping.column_indices[1]][
                            data_row_idx
                        ]
                    ...
                    if other_text:
                        ...
                        repaired_answer = self._repair_mapping.get(
    ...
    def print_semantic_ordering_changes(self):
        ...
        print(
            f"Semantic Ordering Changes Applied ({len(self._ordering_changes)} questions):"
        )
        print("=" * 70)
        ...
        for change in self._ordering_changes:
            ...
            print(
                f"Ordering Type: {change['ordering_type']} (confidence: {change['confidence']:.2f})"
            )
...
if __name__ == "__main__":
    importer = ImportSurveyMonkey(
        "web_takeoff_monadic_results.csv", create_semantic_names=True
    )
