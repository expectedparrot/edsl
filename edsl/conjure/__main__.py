...
def main(
    file_path: Optional[Path] = typer.Argument(
        None, help="Path to the input survey data file (or stdin if not provided)"
    ),
    json_gz_filename: Optional[str] = typer.Option(
        None, "--json-gz-filename", help="Save results to compressed JSON file"
    ),
    silent: bool = typer.Option(False, "--silent", "-s", help="Disable verbose output"),
    sample: Optional[int] = typer.Option(
        None, "--sample", help="Sample size to use instead of processing all agents"
    ),
    data_dictionary_csv: Optional[Path] = typer.Option(
        None,
        "--data-dictionary-csv",
        help="Path to CSV file containing question names and texts",
    ),
    question_name_column: str = typer.Option(
        "question_name",
        "--question-name-column",
        help="Column name or index (0-based) for question names in data dictionary CSV",
    ),
    question_text_column: str = typer.Option(
        "question_text",
        "--question-text-column",
        help="Column name or index (0-based) for question texts in data dictionary CSV",
    ),
    emit_yaml: bool = typer.Option(
        False,
        "--emit-yaml",
        help="Export normalized YAML and agent responses alongside existing processing",
    ),
    emit_yaml_dir: Optional[Path] = typer.Option(
        None,
        "--emit-yaml-dir",
        help="Directory to store questions.yaml and agent_responses.csv (defaults to <file>_normalized)",
    ),
    emit_only: bool = typer.Option(
        False,
        "--emit-only",
        help="Only export YAML/agent responses and skip EDSL conversion",
    ),
    questions_yaml: Optional[Path] = typer.Option(
        None,
        "--from-questions-yaml",
        help="Path to normalized questions YAML to ingest",
    ),
    agent_responses: Optional[Path] = typer.Option(
        None,
        "--agent-responses",
        help="Agent responses file to pair with --from-questions-yaml",
    ),
):
    """
    ...
    try:
        ...
        if questions_yaml:
            if agent_responses is None:
                console.print(
                    "[red]Error:[/red] --agent-responses is required when using --from-questions-yaml"
                )
                sys.exit(1)
            if not questions_yaml.exists():
                console.print(
                    f"[red]Error:[/red] Questions YAML not found: {questions_yaml}"
                )
                sys.exit(1)
            if not agent_responses.exists():
                console.print(
                    f"[red]Error:[/red] Agent responses file not found: {agent_responses}"
                )
                sys.exit(1)
            if emit_yaml or emit_yaml_dir:
                console.print(
                    "[yellow]Warning:[/yellow] --emit-yaml options are ignored when reading from YAML inputs"
                )
            if not silent:
                console.print(
                    f"[blue]Loading normalized survey from:[/blue] {questions_yaml}"
                )
            ...
            if file_path is None:
                if sys.stdin.isatty():
                    console.print(
                        "[red]Error:[/red] No file path provided and no data piped to stdin"
                    )
                    sys.exit(1)
            ...
            if data_dictionary_csv:
                try:
                    if not silent:
                        console.print(
                            f"[blue]Loading data dictionary:[/blue] {data_dictionary_csv}"
                        )
                    ...
                    if not silent:
                        console.print(
                            f"[dim]Data dictionary shape: {dict_df.shape}[/dim]"
                        )
                        console.print(
                            f"[dim]Data dictionary columns: {list(dict_df.columns)}[/dim]"
                        )
                    ...
                    def get_column_data(df, column_spec):
                        if column_spec.isdigit():
                            ...
                            if col_index < 0 or col_index >= len(df.columns):
                                raise ValueError(
                                    f"Column index {col_index} out of range (0-{len(df.columns)-1})"
                                )
                            return df.iloc[:, col_index]
                        else:
                            if column_spec not in df.columns:
                                raise ValueError(
                                    f"Column '{column_spec}' not found in data dictionary CSV"
                                )
                    ...
                    if not silent:
                        console.print(
                            f"[dim]Loaded {len(question_name_to_text)} question mappings[/dim]"
                        )
            ...
            if emit_yaml or emit_yaml_dir:
                if file_path.suffix.lower() != ".csv":
                    console.print(
                        "[yellow]Warning:[/yellow] YAML export currently supports CSV inputs only."
                    )
                    if emit_only:
                        console.print(
                            "[red]Error:[/red] Cannot emit normalized YAML for this file type."
                        )
                        sys.exit(1)
                else:
                    output_dir = (
                        Path(emit_yaml_dir)
                        if emit_yaml_dir
                        else file_path.parent / f"{file_path.stem}_normalized"
                    )
                    ...
                    try:
                        ...
                        console.print(
                            f"[red]Error:[/red] Failed to normalize survey: {e}"
                        )
                        if emit_only:
                            sys.exit(1)
                    else:
                        questions_yaml_path = write_questions_yaml(
                            normalized, output_dir / "questions.yaml"
                        )
                        responses_csv_path = write_agent_responses_csv(
                            normalized, output_dir / "agent_responses.csv"
                        )
                        normalization_success = True
                        if not silent:
                            console.print(
                                f"[green]✓[/green] Wrote normalized questions to {questions_yaml_path}"
                            )
                            console.print(
                                f"[green]✓[/green] Wrote agent responses to {responses_csv_path}"
                            )
                        if emit_only:
                            return
                    if not normalization_success and not silent:
                        console.print(
                            "[yellow]Warning:[/yellow] Normalization was skipped due to earlier errors."
                        )

            conjure_instance = Conjure(
                str(file_path), question_names_to_question_text=question_texts
            )
        ...
        if data_dictionary_csv and question_texts and not silent:
            ...
            actual_columns = set(
                name.lower() for name in conjure_instance.question_names
            )
            ...
            console.print("[cyan]Data Dictionary Usage Diagnostics:[/cyan]")
            console.print(
                f"  📊 Total questions in data dictionary: {len(dict_questions)}"
            )
            ...
            if unmatched_from_dict:
                console.print("  [dim]Dictionary questions not found in data:[/dim]")
                for q in sorted(unmatched_from_dict):
                    console.print(f"    • {q}")

            if unmatched_from_data:
                console.print("  [dim]Data columns not found in dictionary:[/dim]")
                for q in sorted(unmatched_from_data):
                    console.print(f"    • {q}")

            if matched_questions:
                console.print("  [dim]Successfully matched questions:[/dim]")
                for q in sorted(matched_questions):
                    console.print(f"    • {q}")

            match_rate = (
                (len(matched_questions) / len(dict_questions)) * 100
                if dict_questions
                else 0
            )
            ...
            if not question_texts:
                console.print(
                    "[yellow]Warning: Data dictionary CSV was provided but no question texts were loaded[/yellow]"
                )
        ...
        if not silent:
            console.print(
                f"[dim]Created conjure instance of type: {type(conjure_instance).__name__}[/dim]"
            )
