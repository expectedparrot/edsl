...
class QuestionErrorLogger:
    ...
    def __init__(self, datafile_name: str, verbose: bool = False):
        ...
        self.log_file = (
            self.log_dir / f"question_errors_{safe_filename}_{timestamp}.log"
        )

        # Initialize log file
        with open(self.log_file, "w") as f:
            f.write("Question Processing Error Log\n")
    ...
    def _configure_edsl_logging(self):
        ...
        formatter = logging.Formatter(
            "[%(asctime)s] EDSL %(levelname)s: %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
        )
    ...
    def display_summary(self):
        ...
        table = Table(
            title="Question Processing Issues",
            show_header=True,
...
def log_question_error(
    question_name: str, error_type: str, details: str, exception: Exception = None
):
