...
def turn_taking_generator_with_focal_speaker(agent_list, speakers_so_far, focal_speaker_index, **kwargs):
    ...
    if most_recent_speaker == agent_list[focal_speaker_index]:
        non_focal_agents = [a for a in agent_list if a != agent_list[focal_speaker_index]]
        non_focal_speakers = [a for a in speakers_so_far if a != agent_list[focal_speaker_index]]
        return default_turn_taking_generator(agent_list = non_focal_agents, speakers_so_far = non_focal_speakers)
    else:
        return agent_list[focal_speaker_index]
...
def random_inclusive_generator(agent_list, speakers_so_far, **kwargs):
    ...
    if lookback_length == 0:
        ...
        eligible_agents = [a for a in agent_list if a not in speakers_so_far[-lookback_length:]]
...
def speaker_closure(agent_list, generator_function, focal_speaker_index=None):
    ...
    def next_speaker_generator():
        speaker = generator_function(agent_list = agent_list, speakers_so_far = speakers_so_far, focal_speaker_index=focal_speaker_index)
