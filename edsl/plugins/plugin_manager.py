...
class EDSLPluginManager:
    ...
    def install_plugin_from_github(
        self, github_url: str, branch: Optional[str] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            raise PluginInstallationError(
                f"Installation command failed: {e.output.decode() if hasattr(e, 'output') else str(e)}"
            )
    ...
    def _clone_repository(
        self, url: str, target_dir: str, branch: Optional[str] = None
    ) -> None:
        """
    ...
    def _has_package_files(self, directory: str) -> bool:
        ...
        """
        return os.path.exists(os.path.join(directory, "setup.py")) or os.path.exists(
                ...
                if hasattr(plugin, "exports_to_namespace"):
                    ...
                    if exports:
                        ...
                        for name, obj in exports.items():
                            # Log the export
                            logger.info(
                                f"Plugin '{plugin_name}' exports '{name}' to global namespace"
                            )
                ...
                if os.path.exists(plugin_dir):
                    ...
                    if os.path.exists(f"{plugin_module}.py"):
                        spec = importlib.util.spec_from_file_location(
                            plugin_name, f"{plugin_module}.py"
                        )
                        if spec:
                            ...
                            if hasattr(module, plugin_name):
                                ...
                                logger.info(
                                    f"Registered plugin {plugin_name} from {plugin_dir}"
                                )
            except Exception as e:
                logger.warning(
                    f"Error loading plugin {plugin_name} from {plugin_dir}: {str(e)}"
                )
            ...
            try:
                ...
                if methods:
                    for method_name, method in methods.items():
                        ...
                        bound_method = lambda *args, m=method, **kwargs: m(
                            *args, **kwargs
                        )
        ...
        for plugin in self.manager.get_plugins():
            try:
                ...
                plugin_info = {
                    ...
                    "methods": (
                        list(plugin.get_plugin_methods().keys())
                        if hasattr(plugin, "get_plugin_methods")
                        else []
                    ),
