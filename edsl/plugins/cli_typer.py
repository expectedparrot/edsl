...
def list_plugins():
    ...
    for name, info in plugins_info.items():
        ...
        table.add_row(name, info.get("version", "unknown"), description)
...
def list_available_plugins():
    ...
    try:
        ...
        console.print(
            "Use [bold cyan]edsl plugins install <name>[/bold cyan] to install a plugin."
        )
...
def search_for_plugins(
    ...
    try:
        ...
        for plugin in results:
            ...
            table.add_row(plugin.name, plugin.version, tag_str, plugin.description)
...
def show_plugin_info(name: str = typer.Argument(..., help="Plugin name")):
    ...
    if name in local_plugins:
        ...
        panel_content.append(
            f"[bold cyan]Description:[/bold cyan] {info.get('description', 'No description available')}"
        )
        panel_content.append(
            f"[bold cyan]Methods:[/bold cyan] {', '.join(info.get('methods', []))}"
        )
        panel_content.append(
            f"[bold cyan]Installed from:[/bold cyan] {info.get('installed_from', 'Unknown')}"
        )
    ...
    try:
        ...
        panel_content.append(
            f"[bold cyan]Last Updated:[/bold cyan] {info.get('last_update', 'Unknown')}"
        )
        panel_content.append(
            f"[bold cyan]License:[/bold cyan] {info.get('license', 'Unknown')}"
        )
        panel_content.append(
            f"[bold cyan]Dependencies:[/bold cyan] {', '.join(info.get('dependencies', ['None']))}"
        )
        panel_content.append(
            f"[bold cyan]Compatible EDSL versions:[/bold cyan] {', '.join(info.get('compatible_edsl_versions', ['Unknown']))}"
        )
...
def install_plugin(
    ...
    url: Optional[str] = typer.Option(
        None, help="Directly specify GitHub URL instead of using the registry"
    ),
):
    ...
    try:
        ...
        if not github_url:
            ...
            if not github_url:
                console.print(
                    f"[yellow]Plugin '{name}' not found in registry.[/yellow]"
                )
                console.print(
                    "Use [bold cyan]edsl plugins available[/bold cyan] to see available plugins."
                )
                console.print("Or provide the GitHub URL with '--url' if you know it.")
                raise typer.Exit(code=1)

        with console.status(
            f"Installing plugin '{name}' from {github_url}...", spinner="dots"
        ):
            # Install the plugin
            installed_plugins = PluginHost.install_from_github(github_url, branch)

        console.print(
            f"[green]Successfully installed plugin(s): {', '.join(installed_plugins)}[/green]"
        )
        console.print(
...
def uninstall_plugin(name: str = typer.Argument(..., help="Plugin name")):
