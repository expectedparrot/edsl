    ...
    def fix(self, response, verbose=False):
        ...
        try:
            ...
            if numeric_value in self.question_options:
                ...
                if verbose:
                    print(f"Converted string '{response_text}' to integer {numeric_value}")
        ...
        for label, option in label_to_option.items():
            if response_lower == label:
                if verbose:
                    print(f"Exact match found: '{label}' corresponds to option {option}")
        ...
        for label, option in label_to_option.items():
                ...
                for label_word in label_words:
                    if resp_word in label_word or label_word in resp_word:
                        ...
                        if longer > 0:
                            ...
                            score += 25 * similarity
                            if not match_type:
                                match_type = f"Word similarity: '{resp_word}' and '{label_word}'"
            ...
            if verbose:
                print(f"Match score for '{label}' -> {option}: {score} ({match_type if match_type else 'No specific match'})")
        
        # If we found a good match, use it
        if best_score > 0:
            if verbose:
                print(f"Best partial match: '{best_match}' corresponds to option {best_option} (score: {best_score})")
