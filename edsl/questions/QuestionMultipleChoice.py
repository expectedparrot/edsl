...
class QuestionMultipleChoice(QuestionBase):
    ...
    def _translate_answer_code_to_answer(
        self, answer_code: int, substition_dict: Optional[dict] = None
    ):
        ...
        """
        substition_dict = substition_dict or {}
        ...
        if isinstance(self.question_options, str):
            ...
            potential_replacement = substition_dict.get(question_option_key, None)
            # if not isinstance(potential_replacement, list):
            translated_options = potential_replacement

            if translated_options is None:
                # Nope - maybe it's in the substition dict?
                raise ValueError(
                    f"Could not find the key '{question_option_key}' in the scenario."
                    f"The substition dict was: '{substition_dict}.'"
                    f"The question options were: '{self.question_options}'."
                )
        else:
            translated_options = [
                Template(str(option)).render(substition_dict)
