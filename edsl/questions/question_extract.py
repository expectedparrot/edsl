        ...
        for match in re.finditer(json_pattern, text):
            try:
                ...
                if any(key in json_data for key in expected_keys):
                    ...
                    if (
                        len(missing_keys) <= len(expected_keys) // 2
...
class ExtractResponseValidator(ResponseValidatorABC):
    ...
    valid_examples = [
        ...
        (
            {
                "answer": {"name": "Alice", "profession": "Engineer"},
                "comment": "Extracted from text",
            },
    ...
    def fix(self, response, verbose=False):
        ...
        if "generated_tokens" in response and response["generated_tokens"]:
            ...
            extracted_json = extract_json(
                raw_tokens, self.answer_template.keys(), verbose
            )
            if extracted_json:
                if verbose:
        ...
        if "answer" in response and isinstance(response["answer"], str):
            if verbose:
                print(
                    f"Trying to extract from answer field: {response['answer'][:100]}..."
                )

            extracted_json = extract_json(
                response["answer"], self.answer_template.keys(), verbose
            )
            if extracted_json:
                if verbose:
...
class QuestionExtract(QuestionBase):
    ...
    def _simulate_answer(self, human_readable: bool = False) -> dict:
        ...
        return {"answer": simulated_answer, "comment": None, "generated_tokens": None}
