...
class ResultFromInterview:
    ...
    def convert(self) -> "Result":
        ...
        model_response_objects = (
            ...
            if hasattr(self.interview, "valid_results") else []
        )
        
        # Create a copy of the answers
        extracted_answers = (
            dict(self.interview.answers) 
            if hasattr(self.interview, "answers") else {}
        )
        
        # Save essential information from the interview before clearing references
        agent_copy = self.interview.agent.copy() if hasattr(self.interview, "agent") else None
        scenario_copy = (
            self.interview.scenario.copy() if hasattr(self.interview, "scenario") else None
        )
        model_copy = self.interview.model.copy() if hasattr(self.interview, "model") else None
        iteration = self.interview.iteration if hasattr(self.interview, "iteration") else 0
        survey_copy = (
        ...
        question_name_to_prompts = self._get_question_name_to_prompts(model_response_objects)
        prompt_dictionary = (
        ...
        raw_model_results_dictionary, cache_used_dictionary = (
            self._get_raw_model_results_and_cache_used_dictionary(model_response_objects)
        )
