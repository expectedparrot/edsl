...
class Transcripts:
    ...
    def _format_agent_data(self, agent) -> str:
        ...
        if hasattr(agent, "name") and agent.name:
            html_parts.append(
                f'<div style="margin-bottom: 8px;"><strong>Name:</strong> {html_module.escape(str(agent.name))}</div>'
            )

        # Get agent traits if available
        if hasattr(agent, "traits") and agent.traits:
            html_parts.append(
                '<div style="margin-bottom: 8px;"><strong>Traits:</strong></div>'
            )
            ...
            for key, value in agent.traits.items():
                ...
                html_parts.append(
                    f'<div style="margin-bottom: 6px;"><strong>{html_module.escape(str(key))}:</strong> {html_module.escape(value_str)}</div>'
                )
            ...
            if scenario_dict:
                for key, value in scenario_dict.items():
                    ...
                    if len(value_str) > 500:
                        value_str = value_str[:497] + "..."
                    html_parts.append(
                        f'<div style="margin-bottom: 8px;"><strong>{html_module.escape(str(key))}:</strong> {html_module.escape(value_str)}</div>'
                    )
