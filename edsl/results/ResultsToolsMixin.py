class ResultsToolsMixin:
    ...
    def get_themes(self, 
        ...
        if len(values) > max_values:
            print(f"Warning: too many values ({len(values)}). Truncating to {max_values}.")
            import random
            values = random.sample(values, max_values)
        from edsl.questions import QuestionList
        from edsl import ScenarioList
        q = QuestionList(question_text = f"""
        {context}
    ...
    def answers_to_themes(self, field, context: str, themes: list, progress_bar = False) -> dict:
        from edsl import QuestionCheckBox, ScenarioList
        values = self.select(field).to_list()
        scenarios = ScenarioList.from_list("field", values).add_value("context", context)
    ...
    def auto_theme(self, field: str, context: str, num_themes: int = 10, seed = None, progress_bar = False):
        themes = self.get_themes(field, context, num_themes = num_themes, seed = seed)
        answers_to_themes = self.answers_to_themes(field, context, themes, progress_bar = progress_bar)
