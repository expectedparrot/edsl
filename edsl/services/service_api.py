"""
Generic FastAPI application that exposes service classes as REST APIs.

Works with any class that follows the service pattern:
- Has `service_name` attribute
- Has `extends` attribute listing types it extends
- Has methods decorated with `@method_type()` decorator
"""
...
def get_service_info(cls: Type) -> dict:
    ...
    Returns:
        dict: A dictionary containing service_name, description, and methods info
    """
    import re
...
class ServiceAPI:
    ...
    Usage:
        ...
        api = ServiceAPI()
        api.register_service(FirecrawlService)
        app = api.create_app()
        
        # Run with: uvicorn service_api:app --reload
    """
...
# Default app creation for direct usage
...
if __name__ == "__main__":
    ...
    api = ServiceAPI()
    api.register_service(FirecrawlService)
    app = api.create_app(title="Firecrawl Service API")
    
    uvicorn.run(app, host="0.0.0.0", port=8008)
else:
    # For import: create app with FirecrawlService if available
    try:
        from firecrawl_service import FirecrawlService
        api = ServiceAPI()
        api.register_service(FirecrawlService)
        ...
        # Create empty app if no services available
