...
class ServiceMetadata:
    ...
    operations: Dict[str, OperationSchema] = field(default_factory=dict)  # Operation schemas
...
class ServiceRegistry:
    ...
    ) -> Union[Type["ExternalService"], Callable[[Type["ExternalService"]], Type["ExternalService"]]]:
        """
            ...
            if aliases:
                for alias in aliases:
                    if alias in cls._aliases or alias in cls._services:
                        raise ValueError(f"Alias '{alias}' conflicts with existing name")
    ...
    @classmethod
    def get_operation_schema(cls, name: str, operation: str) -> Optional[OperationSchema]:
        """
    ...
    def info(cls, name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "operations": list(meta.operations.keys()) if meta and meta.operations else [],
        }
            ...
            for ep in eps:
                try:
                    ...
                    if ep.name not in cls._services:
                        ...
                        from .base import ExternalService
                        if isinstance(service_class, type) and issubclass(service_class, ExternalService):
