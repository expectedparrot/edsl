...
class ScenarioInspectorWidget(InspectorWidget):
    ...
    def _validate_object(self, obj) -> bool:
        ...
        if obj is None:
            return True
        return hasattr(obj, 'data') and hasattr(obj, 'name') or type(obj).__name__ == 'Scenario'
    ...
    def _enhance_summary(self, summary: Dict[str, Any]):
        ...
        scenario_data = {k: v for k, v in self.data.items() 
        ...
        summary.update({
            ...
            'variable_count': len(scenario_data) if isinstance(scenario_data, dict) else 0,
