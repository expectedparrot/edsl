...
class DataFrameGridChartsWidget(EDSLBaseWidget):
    ...
    # Chart configuration
    chart_type = traitlets.Unicode("bar").tag(
        sync=True
    ...
    layout_mode = traitlets.Unicode("tabs").tag(
        sync=True
    )  # tabs, split, grid-only, charts-only
    ...
    def _prepare_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for col in df_clean.columns:
            if df_clean[col].dtype == "object":
                ...
                df_clean[col] = df_clean[col].fillna(
                    0
                )  # Use 0 instead of empty string for charts
    ...
    def _set_default_chart_columns(self):
        ...
        if not self.chart_x_column and (
            self.categorical_columns or self.datetime_columns
        ):
