...
class ResultsViewerWidget(EDSLBaseWidget):
    ...
    def _update_data(self):
        ...
        try:
            ...
            if hasattr(self.results, "survey") and hasattr(
                self.results.survey, "questions"
            ):
                for question in self.results.survey.questions:
                    ...
                    if question_info["question_type"] in [
                        ...
                        try:
                            values = self.results.select(
                                question.question_name
                            ).to_list()

                            # Store raw data for cross-tabulation
                            raw_data[question.question_name] = [
                                str(val) if val is not None else None for val in values
                            ]
                            ...
                            if question_info["question_type"] == "numerical":
                                ...
                                value_counts = Counter(
                                    [str(val) for val in values if val is not None]
                                )
                                chart_data[question.question_name] = dict(value_counts)

                        except Exception as e:
                            print(
                                f"Error extracting data for {question.question_name}: {e}"
                            )
                            chart_data[question.question_name] = (
                                []
                                if question_info["question_type"] == "numerical"
                                else {}
                            )
