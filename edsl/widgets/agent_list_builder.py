...
class AgentListBuilderWidget(EDSLBaseWidget):
        ...
        try:
            ...
            if hasattr(agent_list, '__iter__'):
                ...
                for i, agent in enumerate(agent_list):
                    ...
                    agent_name = agent.name if hasattr(agent, 'name') and agent.name else f"Agent_{i+1}"
        ...
        for agent in self.agent_list_data['agents']:
            ...
            for trait, filter_config in self.trait_filters.items():
                ...
                if trait_type == 'number':
                    try:
                        trait_value = float(trait_value) if trait_value is not None else None
                    ...
                    trait_value = str(trait_value).lower() in ['true', '1', 'yes', 'on'] if trait_value is not None else False
                    filter_value = str(filter_value).lower() in ['true', '1', 'yes', 'on'] if filter_value else False
                else:
                ...
                # Apply filter logic
                if not self._evaluate_filter(trait_value, filter_type, filter_value, filter_values, trait_type):
                    matches = False
                    break
    ...
    def _get_trait_type(self, value):
        ...
        if isinstance(value, bool) or str(value).lower() in ['true', 'false', '1', '0', 'yes', 'no', 'on', 'off']:
            return 'boolean'
        
        return 'string'

    def _evaluate_filter(self, trait_value, filter_type, filter_value, filter_values, trait_type):
        ...
        try:
            if filter_type == 'equals':
                ...
                if trait_type == 'string':
                    return any(str(trait_value).lower() == str(v).lower() for v in filter_values)
                else:
                    return trait_value in filter_values
            elif filter_type == 'not_in':
                if trait_type == 'string':
                    return not any(str(trait_value).lower() == str(v).lower() for v in filter_values)
                else:
    ...
    def _apply_percentage_sample(self, percentage):
        ...
        if percentage >= 100:
            ...
            self.selected_agents = random.sample(self.filtered_agents, min(sample_size, len(self.filtered_agents)))
    ...
    def _toggle_agent_selection(self, agent_name):
        ...
        current_selected = [agent for agent in self.selected_agents if agent.get('name') == agent_name]
        
        if current_selected:
            # Remove from selection
            self.selected_agents = [agent for agent in self.selected_agents if agent.get('name') != agent_name]
        else:
            # Add to selection
            agent_to_add = None
    ...
    def _update_stats(self):
        ...
        self.stats = {
            ...
            'active_filter_count': len([f for f in self.trait_filters.values() if f.get('enabled', False)])
        }
    ...
    def _update_widget_data_after_agent_list_change(self):
        ...
        try:
            ...
            current_selected_agent_names = [agent.get('name', '') for agent in self.selected_agents]
            current_filtered_agent_names = [agent.get('name', '') for agent in self.filtered_agents]
            ...
            self.agent_list_data = {
                ...
                'visibility': getattr(self._original_agent_list, 'visibility', 'private'),
            ...
            self.available_traits = sorted(list(all_traits))
            self.selected_traits = [trait for trait in self.selected_traits if trait in all_traits]
            ...
            self.filtered_agents = [agent for agent in agents_data 
                                  if agent.get('name', '') in current_filtered_agent_names]
            self.selected_agents = [agent for agent in agents_data 
        ...
        try:
            ...
            for agent_data in self.selected_agents:
                ...
                if agent_data.get('traits_presentation_template'):
                    agent._traits_presentation_template = agent_data['traits_presentation_template']
    ...
    def _update_original_agent_list(self):
        ...
        try:
            ...
            for agent_data in self.selected_agents:
                ...
                filtered_traits = {trait: all_traits[trait] for trait in self.selected_traits if trait in all_traits}
