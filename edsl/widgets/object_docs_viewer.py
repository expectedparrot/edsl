...
class ObjectDocsViewerWidget(EDSLBaseWidget):
    ...
    def _format_docstring(self, docstring: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            elif in_example and stripped and not stripped.startswith('>>>') and not stripped.startswith('...'):
    ...
    def _calculate_bm25_score(self, query_tokens: List[str], doc_tokens: List[str], 
    ...
    def _highlight_matches(self, text: str, query_tokens: List[str]) -> List[Dict[str, Any]]:
    ...
    def _perform_search(self, query: str):
        ...
        try:
            ...
            for method in self.methods_data:
                ...
                searchable_text = f"{method['name']} {method['signature']} {method['docstring']}"
            ...
            results = []
            for i, (method, doc_tokens) in enumerate(zip(self.methods_data, doc_tokens_list)):
                score = self._calculate_bm25_score(query_tokens, doc_tokens, corpus_stats)
                
                if score > 0:  # Only include methods with matches
                    # Highlight matches in different parts
                    highlighted_name = self._highlight_matches(method['name'], query_tokens)
                    highlighted_signature = self._highlight_matches(method['signature'], query_tokens)
                    highlighted_docstring = self._highlight_matches(method['docstring'], query_tokens)
