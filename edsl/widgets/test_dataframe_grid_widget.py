...
def test_data_processing():
    ...
    for name, df in dataframes.items():
        ...
        assert len(widget.columns) == len(df.columns) + 1, f"Column count mismatch for {name} DataFrame (+1 for row index)"
        
        print(f"  ✓ {name} DataFrame processed successfully ({len(df)} rows, {len(df.columns)} columns)")
