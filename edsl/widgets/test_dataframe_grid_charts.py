...
def create_sample_data():
    ...
    data = {
        ...
        'Seasonal_Factor': np.sin(np.arange(200) * 2 * np.pi / 365) + np.random.normal(0, 0.1, 200),
...
def test_basic_functionality():
    ...
    print(f"   • Numeric columns: {len(widget.numeric_columns)} - {widget.numeric_columns[:3]}...")
...
def test_chart_configuration():
    ...
    for chart_type in chart_types:
        ...
        assert 'series' in options, f"Chart options should contain series for {chart_type}"
        assert len(options['series']) > 0, f"Should have at least one series for {chart_type}"
...
def test_column_analysis():
    ...
    mixed_data = {
        ...
        'high_cardinality': [f'item_{i}' for i in range(5)]  # Should not be categorical
    }
    ...
    assert 'category_col' in widget.categorical_columns, "Low cardinality string should be categorical"
