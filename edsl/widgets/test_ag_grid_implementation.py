...
def test_column_definitions():
    ...
    assert len(visible_columns) == len(df.columns), f"Should have {len(df.columns)} visible columns"
    
    # Check that all DataFrame columns are represented
    df_columns = set(df.columns)
    widget_columns = set(col['field'] for col in visible_columns)
    assert df_columns == widget_columns, f"Column mismatch: {df_columns} vs {widget_columns}"
    ...
    assert len(sortable_columns) == len(visible_columns), "All columns should be sortable by default"
    assert len(filterable_columns) == len(visible_columns), "All columns should be filterable by default"
