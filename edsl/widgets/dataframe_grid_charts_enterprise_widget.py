...
class DataFrameGridChartsEnterpriseWidget(EDSLBaseWidget):
    ...
    chart_themes = traitlets.List(
        default_value=["ag-default", "ag-material", "ag-solar"]
    ).tag(sync=True)
    ...
    def _process_dataframe(self):
        ...
        try:
            ...
            self.status = "ready"
            logger.info(
                f"Processed DataFrame for Enterprise features: {len(self.dataframe)} rows, {len(self.dataframe.columns)} columns"
            )
    ...
    def _prepare_enterprise_columns(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for col in df.columns:
            ...
            if pd.api.types.is_integer_dtype(dtype) or pd.api.types.is_float_dtype(
                dtype
            ):
                col_def.update(
                    {
