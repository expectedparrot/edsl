...
try:
    ...
    print(
        "Failed to import DataFrameGridWidget. Make sure the widget is properly installed."
    )
    sys.exit(1)
...
def create_demo_dataframes():
    ...
    sales_data["Total_Value"] = (
        sales_data["Price"] * sales_data["Quantity"] * (1 - sales_data["Discount"])
    )
    ...
    return {"sales": sales_data, "employees": employee_data}
...
def demo_advanced_features():
    ...
    print(
        f"  - Configuration: Page size={widget.page_size}, Selection={widget.selection_mode}"
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "Description": [
                f"Description for item {i} with some longer text to test column width"
                for i in range(1, 11)
            ],
...
def demo_error_handling():
    ...
    try:
        ...
        print(
            f"✓ Invalid input handled: Status={widget.status}, Error='{widget.error_message}'"
        )
    except:
    ...
    print(
        f"✓ Empty DataFrame handled: Status={widget.status}, Error='{widget.error_message}'"
    )
