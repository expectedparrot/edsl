...
class TemplateReportGenerator:
        ...
        *fields: Optional[str],
        ...
        if filestore and not filename:
            ...
            if explode:
                ...
                effective_filename = os.path.join(temp_dir, f"report_{{index}}{extension}")
            else:
        ...
        if explode:
            result = self._handle_explode_mode(
                rendered_reports, list_of_dicts, title_template, format, effective_filename
            )
            if filestore:
                return self._wrap_explode_result_in_filestore(result, effective_filename)
            return result
        ...
        result = self._handle_combined_mode(
            ...
            effective_filename, separator, return_string
        )
        if filestore:
            return self._wrap_combined_result_in_filestore(result, effective_filename, format)
        return result
    ...
    def _convert_with_pandoc(self, markdown_content: str, output_format: str, temp_dir: str = None) -> Union[str, "Document"]:
        ...
        output_extension = format_extensions.get(output_format.lower(), f".{output_format.lower()}")
        ...
        try:
            ...
            if os.path.exists(output_filename):
                os.unlink(output_filename)
            from .exceptions import DatasetExportError
            stderr_output = e.stderr.decode() if e.stderr else "No error details available"
            raise DatasetExportError(f"Pandoc conversion to {output_format} failed: {stderr_output}")
    ...
    def _handle_combined_mode(
        self, rendered_reports: List[str], list_of_dicts: List[dict], 
        title_template, format: str, filename: Optional[str], 
        separator: str, return_string: bool
        ...
        if format.lower() == "text":
            ...
            return self._create_combined_with_pandoc(
                rendered_reports, list_of_dicts, title_template, 
                format, filename, return_string
            )

    def _create_combined_with_pandoc(
        self, rendered_reports: List[str], list_of_dicts: List[dict], 
        title_template, format: str, filename: Optional[str], return_string: bool
    ) -> Optional[Union[str, "Document", bytes]]:
    ...
    def _wrap_explode_result_in_filestore(self, result: List, filename_template: Optional[str]) -> List["FileStore"]:
    ...
    def _wrap_combined_result_in_filestore(self, result, filename: Optional[str], format: str) -> "FileStore":
