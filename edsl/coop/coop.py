...
class Coop(CoopFunctionsMixin):
    ...
    def create_encrypted_proxy_key(
        ...
        """
        Create an encrypted proxy key using the current API key.
        
        This method encrypts the current Expected Parrot API key along with usage
        parameters using Expected Parrot's public GPG key. The resulting encrypted
        proxy key can be used for limited access with specific constraints.
        ...
        Args:
            ...
            ep_public_key: Expected Parrot's public GPG key (optional, uses default if not provided)
        ...
        Raises:
            ...
            RuntimeError: If GPG encryption fails
            
        Example:
            >>> coop = Coop()
            >>> proxy_key = coop.create_encrypted_proxy_key(
            ...     max_charge_credits=500.0,
            ...     duration_seconds=3600
            ... )
            >>> print(f"Encrypted proxy key: {proxy_key}")
        """
        if not self.api_key:
            raise ValueError("No Expected Parrot API key available. Please set your API key first.")
            
        from .proxy_key_encryption import ProxyKeyEncryption
        
        encryptor = ProxyKeyEncryption(ep_public_key)
        return encryptor.encrypt_proxy_key(
            ep_token=self.api_key,
            max_charge_credits=max_charge_credits,
            duration_seconds=duration_seconds
        )
        
    def decrypt_proxy_key(self, encrypted_proxy_key: str):
        """
        Decrypt a proxy key and return its payload information.
        ...
        Example:
            ...
            >>> print(f"Token: {payload.ep_token}")
            >>> print(f"Max credits: {payload.max_charge_credits}")
        """
        from .proxy_key_encryption import ProxyKeyEncryption
        
        encryptor = ProxyKeyEncryption()
        return encryptor.decrypt_proxy_key(encrypted_proxy_key)
        
    def register_proxy_key_for_tracking(self, encrypted_proxy_key: str) -> str:
        """
        Register a proxy key for charge tracking.
        ...
        Example:
            >>> coop = Coop()
            >>> key_hash = coop.register_proxy_key_for_tracking(encrypted_key)
            >>> print(f"Registered key: {key_hash}")
        """
        from .proxy_key_tracker import ProxyKeyTracker
        
        tracker = ProxyKeyTracker()
        return tracker.register_proxy_key(encrypted_proxy_key)
        
    def check_proxy_key_balance(self, encrypted_proxy_key: str):
        """
        Check the balance and status of a proxy key.
        ...
        Example:
            >>> coop = Coop()
            >>> balance = coop.check_proxy_key_balance(encrypted_key)
    ...
    def charge_proxy_key(
        ...
        """
        Charge an amount against a proxy key.
        ...
        Example:
            >>> coop = Coop()
            >>> success = coop.charge_proxy_key(encrypted_key, 10.5, "API call")
            >>> if success:
    ...
    def get_proxy_key_charge_history(
        self,
        encrypted_proxy_key: str,
        ...
        Example:
            >>> coop = Coop()
            >>> history = coop.get_proxy_key_charge_history(encrypted_key, limit=10)
            >>> for charge in history:
            ...     print(f"{charge.charge_date}: {charge.charge_amount} - {charge.description}")
        """
        from .proxy_key_tracker import ProxyKeyTracker
        
        tracker = ProxyKeyTracker()
        return tracker.get_charge_history(encrypted_proxy_key, limit)
