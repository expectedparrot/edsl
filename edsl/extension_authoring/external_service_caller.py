...
class ExternalServiceCaller:
    ...
    def call_service(
        ...
        logger.debug("Sending request to external endpoint %s | Headers: %s | Payload: %s",
                    endpoint, headers, params)
        
        try:
            response = requests.post(endpoint, json=params, headers=headers, timeout=self.timeout)
            
            logger.debug("Received response from %s | Status: %s", endpoint, response.status_code)
            ...
            logger.error("External service request failed for %s at %s: %s",
                        service_def.service_name, endpoint, e)
            raise ServiceConnectionError(
                f"Error connecting to external service '{service_def.service_name}': {str(e)}"
            ) from e
        except requests.exceptions.RequestException as e:
            logger.error("Unexpected error calling external service %s at %s: %s",
                        service_def.service_name, endpoint, e)
            raise ServiceConnectionError(
...
class AsyncExternalServiceCaller:
    ...
    async def call_service(
        ...
        logger.debug("Sending async request to external endpoint %s | Headers: %s | Payload: %s",
                    endpoint, headers, params)
        
        try:
            response = await self.http_client.post(endpoint, json=params, headers=headers)
            ...
            if "httpx" in str(type(e)) and "RequestError" in str(type(e)):
                ...
                logger.exception("Error processing response from external service '%s'",
                               service_def.service_name)
                # Convert to FastAPI HTTPException for gateway compatibility
                try:
                    from fastapi import HTTPException
