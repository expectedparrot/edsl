...
class BaseServiceCaller(ABC):
    ...
    def __init__(self, service_name: str, ep_api_token: Optional[str] = None, timeout: int = TIMEOUT):
...
class GatewayServiceCaller(BaseServiceCaller):
    ...
    def __init__(self, service_name: str, gateway_url: str, ep_api_token: Optional[str] = None, timeout: int = TIMEOUT):
    ...
    def call_service(self, prepared_params: Dict[str, Any], deserialize_response: Callable) -> Any:
        ...
        if not self.gateway_url:
            raise ServiceConfigurationError(f"Service '{self.service_name}' cannot be called. Configuration missing (gateway_url).")
...
class DirectServiceCaller(BaseServiceCaller):
    ...
    def __init__(self, service_name: str, service_endpoint: str, ep_api_token: Optional[str] = None, timeout: int = 120):
        super().__init__(service_name, ep_api_token, timeout)
    ...
    def call_service(self, prepared_params: Dict[str, Any], deserialize_response: Callable) -> Any:
        ...
        logger.info("Calling service '%s' directly at %s...", self.service_name, self.service_endpoint)
