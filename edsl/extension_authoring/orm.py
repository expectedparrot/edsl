...
class ServiceDefinitionORM(Base):
    ...
    # Relationships
    parameters = relationship("ParameterDefinitionORM", back_populates="service_definition", cascade="all, delete-orphan")
    cost = relationship("CostDefinitionORM", back_populates="service_definition", uselist=False, cascade="all, delete-orphan")
    service_returns = relationship("ReturnDefinitionORM", back_populates="service_definition", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f"<ServiceDefinitionORM(id={self.id}, service_name='{self.service_name}')>"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "service_returns": {ret.name: ret.to_dict() for ret in self.service_returns}
        }
...
class ParameterDefinitionORM(Base):
    ...
    description = Column(Text, nullable=False)
    default_value = Column(JSON, nullable=True)  # Using JSON to store any type of default value
    
    # Relationships
    service_definition = relationship("ServiceDefinitionORM", back_populates="parameters")
...
class ReturnDefinitionORM(Base):
    ...
    # Relationships
    service_definition = relationship("ServiceDefinitionORM", back_populates="service_returns")
...
class ServiceDefinitionRepository:
    ...
    def get_by_name(self, name: str) -> Optional[ServiceDefinitionORM]:
        ...
        return self.session.query(ServiceDefinitionORM).filter_by(service_name=name).first()
