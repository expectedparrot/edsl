...
class HeadAttachments:
    ...
    def apply_formatter(
        ...
        logger.info(f"apply_formatter called: formatter.target={formatter.target}, formatter.description={getattr(formatter, 'description', None)}")
        logger.info(f"Current attachments before: scenario={self.scenario is not None}, survey={self.survey is not None}, agent_list={self.agent_list is not None}")
        ...
        logger.info(f"Starting value for target '{formatter.target}': {type(starting_value).__name__ if starting_value else None}")
        ...
        if isinstance(rendered, (Scenario, ScenarioList)):
            logger.info(f"Rendered is Scenario/ScenarioList, setting to scenario slot")
            ...
            logger.info(f"Rendered is other type, falling back to target slot: {formatter.target}")
        ...
        logger.info(f"Attachments after formatter: scenario={self.scenario is not None}, survey={self.survey is not None}, agent_list={self.agent_list is not None}")
        return self

    def attach_to_head(self, jobs: "Jobs") -> "Jobs":
        logger.info(f"attach_to_head called with scenario={self.scenario is not None}, survey={self.survey is not None}, agent_list={self.agent_list is not None}")
        if self.scenario:
        ...
        if self.agent_list:
            logger.info(f"Adding agent_list to head: type={type(self.agent_list).__name__}")
