...
from edsl.questions import QuestionList, QuestionFreeText, QuestionEDSLObject, QuestionCheckBox
...
def create_theme_generation_app():
    ...
    jobs_object = (
        Survey([theme_generation_question])
        .to_jobs()
    )
...
def create_theme_finder_app():
    ...
    jobs_object = (
        Survey([candidate_theme_question])
        .to_jobs()
        .by(agent)
    )
    ...
    return App(
        application_name="Theme Finder",
        description="Generates candidate themes from all responses in parallel",
        initial_survey=initial_survey,
        jobs_object=jobs_object,
...
def create_complete_theme_finder():
    ...
    return App(
        application_name="Complete Theme Finder",
        description="Full theme extraction pipeline with generation, consolidation, and labeling",
        initial_survey=initial_survey,
        jobs_object=jobs_object,
        output_formatters={"labeled": labeled_formatter},
        default_formatter_name="labeled",
...
if __name__ == "__main__":
    ...
    candidate_themes_sl = theme_generator.output({
        "responses": responses_sl,
        "target_theme_count": "5"
    })

    print(f"✓ Generated {len(candidate_themes_sl)} candidate themes from {len(responses_sl)} responses")
    ...
    consolidated_themes = consolidator.output({
    ...
    for i, theme in enumerate(consolidated_themes, 1):
        print(f"  {i}. {theme.get('label', 'Unnamed')}: {theme.get('description', 'No description')}")
    ...
    labeled_responses_sl = labeler.output({
        "responses": responses_with_guide_sl,
        "themes_guide": themes_guide
    })
    ...
    print(f"Average themes per response: {total_labels / len(labeled_responses_sl):.2f}")
