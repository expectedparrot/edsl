"""AppDeploy dataclass for managing EDSL app deployment configuration."""
...
class ExecutionMode(str, Enum):
    """Where and how the app can be executed."""
...
class PricingConfig:
    ...
    per_call_price: Optional[float] = None  # Price per app execution
...
class AppDeploy:
    """
    Deployment configuration for an EDSL App.

    This dataclass holds all metadata and settings relevant to deploying
    and distributing an EDSL application through Coop or other channels.

    Attributes:
        execution_mode: How users can execute the app (online, downloadable, or both)
        ...
        author: Information about the app creator
        pricing: Pricing configuration
        executor: Who runs the app ("creator" uses creator's resources, "user" uses user's)
        requires_coop: Whether app requires Coop infrastructure
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "AppDeploy":
        """Create AppDeploy instance from dictionary."""
    ...
    @classmethod
    def example_free_open_source(cls) -> "AppDeploy":
        """Example: Free, open-source app with downloadable code."""
        return cls(
            execution_mode=ExecutionMode.HYBRID,
            source_available=True,
            license="MIT",
    ...
    @classmethod
    def example_commercial_hosted(cls) -> "AppDeploy":
        """Example: Commercial app, hosted only, per-call pricing."""
        return cls(
    ...
    @classmethod
    def example_token_markup(cls) -> "AppDeploy":
        """Example: App with token markup pricing model."""
        return cls(
            execution_mode=ExecutionMode.HYBRID,
            source_available=True,
            license="Apache-2.0",
...
if __name__ == "__main__":
    ...
    deploy1 = AppDeploy.example_free_open_source()
    print(deploy1)
    print("\nSerialized:")
    ...
    deploy2 = AppDeploy.example_commercial_hosted()
    print(deploy2)

    print("\n" + "="*50)
    print("Example 3: Token Markup")
    deploy3 = AppDeploy.example_token_markup()
    print(deploy3)
    ...
    restored = AppDeploy.from_dict(data)
