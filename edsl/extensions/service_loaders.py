...
class APIServiceLoader(ServiceLoader):
    ...
    def load_services(self) -> List[Dict[str, Any]]:
        ...
        logger.info("Fetching service configurations from %s...", self.url)
        try:
            ...
            if not services_list:
                logger.warning("No services found at the specified endpoint.")
            return services_list
        except requests.exceptions.RequestException as e:
            logger.error("Error fetching service configurations via API: %s. Services will be unavailable.", e)
            return []
        except json.JSONDecodeError as e:
            logger.error("Error decoding JSON response from %s: %s. Services will be unavailable.", self.url, e)
            return []
...
class GithubYamlLoader(ServiceLoader):
    ...
    def _fetch_directory_contents(self, api_url: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if not isinstance(contents, list):
                logger.warning("Expected a list of files from GitHub API at %s, but got %s. Cannot load services.", api_url, type(contents))
                return None
            return contents
        except requests.exceptions.RequestException as e:
            logger.error("Error fetching directory contents from GitHub API (%s): %s. Services will be unavailable.", api_url, e)
            return None
        except json.JSONDecodeError as e:
            logger.error("Error decoding JSON response from GitHub API (%s): %s. Services will be unavailable.", api_url, e)
            return None
        except Exception as e: # Catch other potential errors
             logger.error("An unexpected error occurred fetching directory contents from GitHub (%s): %s", api_url, e)
             return None
    ...
    def _fetch_and_parse_yaml_file(self, file_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not file_url:
            logger.warning("Could not get download URL for file '%s'. Skipping.", file_name)
            return None

        try:
            logger.debug("Fetching file %s...", file_name)
            ...
            if isinstance(service_data, dict):
                ...
                logger.warning("Could not parse a valid service definition (dictionary) from '%s'. Got %s. Skipping file.", file_name, type(service_data))
                return None

        except requests.exceptions.RequestException as e:
            logger.error("Error fetching file '%s' from %s: %s", file_name, file_url, e)
            return None
        except yaml.YAMLError as e:
            logger.error("Error parsing YAML file '%s': %s", file_name, e)
            return None
        except Exception as e:
            logger.error("An unexpected error occurred processing file '%s': %s", file_name, e)
            return None
    ...
    def load_services(self) -> List[Dict[str, Any]]:
        ...
        logger.info("Fetching service definitions from GitHub: %s/%s/%s", self.repo_owner, self.repo_name, self.directory_path)
        ...
        if not yaml_files_info:
            logger.warning("No YAML files found in %s/%s/%s", self.repo_owner, self.repo_name, self.directory_path)
            return []
        ...
        logger.info("Successfully loaded %d service definition(s) from GitHub.", len(loaded_services))
