...
class ServiceResponseProcessor:
    ...
    def deserialize_single_value(self, return_key: str, return_def: Any, response_data: Dict[str, Any]) -> Any:
        """Deserializes a single value from the response data based on its definition."""
        raw_value = response_data.get(return_key)
        if raw_value is None:
            raise ServiceDeserializationError(f"Expected return key '{return_key}' not found in response for service '{self.service_name}'.")
        ...
        # Check standard Python types
            ...
            raise ServiceDeserializationError(f"Unrecognized return type '{return_type_str}' defined for key '{return_key}' in service '{self.service_name}'.")
    ...
    def deserialize_response(self, response: requests.Response) -> Any:
        ...
        try:
            for return_key, return_def in self.service_returns.items():
                deserialized_value = self.deserialize_single_value(return_key, return_def, response_data)
                deserialized_results[return_key] = deserialized_value
        except ServiceDeserializationError as e:
             raise e
        except Exception as e:
             raise ServiceDeserializationError(f"Unexpected error during response deserialization for service '{self.service_name}': {e}") from e
