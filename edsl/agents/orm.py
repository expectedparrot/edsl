"""SQLAlchemy ORM models for the Agent and AgentList domain objects.

This module provides SQLAlchemy ORM models for persisting Agent and AgentList
...
agent_list_agent_association = Table(
    ...
    Base.metadata,
    Column("agent_list_id", Integer, ForeignKey("agent_lists.id", ondelete="CASCADE"), primary_key=True),
...
class SQLAgentTrait(Base):
    ...
    def __repr__(self) -> str:
        ...
        return f"<SQLAgentTrait(id={self.id}, agent_id={self.agent_id}, key='{self.key}')>"
...
class SQLAgent(Base):
    ...
    has_dynamic_traits_function = Column(String(1), nullable=False, default="0")  # "0" or "1"
    ...
    # Relationships
    traits = relationship("SQLAgentTrait", back_populates="agent", cascade="all, delete-orphan")
    codebook_entries = relationship("SQLAgentCodebook", back_populates="agent", cascade="all, delete-orphan")
    ...
    def serialize_value(value: Any) -> tuple[str, str]:
        ...
        if value is None:
            ...
            try:
                ...
                raise AgentOrmException(f"Could not serialize value of type {type(value)}: {e}")
    ...
    def to_agent(self) -> Agent:
        ...
        if self.has_dynamic_traits_function == "1" and self.dynamic_traits_function_source_code:
        ...
        if self.answer_question_directly_source_code and self.answer_question_directly_function_name:
            from ..utilities import create_restricted_function
            import types
            ...
            agent.answer_question_directly_function_name = self.answer_question_directly_function_name
    ...
    def from_agent(cls, agent: Agent, session: Optional[Session] = None) -> SQLAgent:
        ...
        orm_agent = cls(
            ...
            has_dynamic_traits_function="1" if getattr(agent, 'has_dynamic_traits_function', False) else "0",
        ...
        if hasattr(agent, 'dynamic_traits_function') and agent.dynamic_traits_function:
            import inspect
            try:
                orm_agent.dynamic_traits_function_source_code = inspect.getsource(agent.dynamic_traits_function)
        ...
        if hasattr(agent, 'answer_question_directly'):
            import inspect
            try:
                orm_agent.answer_question_directly_source_code = inspect.getsource(agent.answer_question_directly)
            except (TypeError, OSError):
                print(f"Warning: Could not get source code for answer_question_directly function")
...
def update_agent(session: Session, agent_id: int, agent: Agent) -> bool:
    ...
    agent_orm.traits_presentation_template = getattr(agent, 'traits_presentation_template', None)
    agent_orm.has_dynamic_traits_function = "1" if getattr(agent, 'has_dynamic_traits_function', False) else "0"
    agent_orm.dynamic_traits_function_name = getattr(agent, 'dynamic_traits_function_name', None)
    agent_orm.answer_question_directly_function_name = getattr(agent, 'answer_question_directly_function_name', None)
    
    # Update function source code if available
    if hasattr(agent, 'dynamic_traits_function') and agent.dynamic_traits_function:
        import inspect
        try:
            agent_orm.dynamic_traits_function_source_code = inspect.getsource(agent.dynamic_traits_function)
        except (TypeError, OSError):
            print(f"Warning: Could not get source code for dynamic traits function")
    ...
    if hasattr(agent, 'answer_question_directly'):
        import inspect
        try:
            agent_orm.answer_question_directly_source_code = inspect.getsource(agent.answer_question_directly)
        except (TypeError, OSError):
            print(f"Warning: Could not get source code for answer_question_directly function")
...
def list_agents(session: Session, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
    """List agents in the database with pagination."""
    agents = session.query(SQLAgent).order_by(SQLAgent.created_at.desc()) \
        .limit(limit).offset(offset).all()
    return [{"id": a.id, "name": a.name, "created_at": a.created_at} for a in agents]


def list_agent_lists(session: Session, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
    """List agent lists in the database with pagination."""
    lists = session.query(SQLAgentList).order_by(SQLAgentList.created_at.desc()) \
        .limit(limit).offset(offset).all()
    return [{"id": l.id, "name": l.name, "created_at": l.created_at,
             "agent_count": len(l.agents)} for l in lists]
