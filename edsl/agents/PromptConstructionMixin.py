...
class PromptPlan:
    ...
    def __init__(self, user_prompt_order: Optional[tuple]=None, system_prompt_order: Optional[tuple]=None):
        """Initialize the PromptPlan."""

        if user_prompt_order is None:
            user_prompt_order = (PromptComponent.QUESTION_INSTRUCTIONS, PromptComponent.PRIOR_QUESTION_MEMORY)
        if system_prompt_order is None:
            system_prompt_order = (PromptComponent.AGENT_INSTRUCTIONS, PromptComponent.AGENT_PERSONA)
        ...
        if not isinstance(user_prompt_order, tuple):
            raise TypeError(f"Expected a tuple, but got {type(user_prompt_order).__name__}")

        if not isinstance(system_prompt_order, tuple):
            raise TypeError(f"Expected a tuple, but got {type(system_prompt_order).__name__}")
        ...
        if not self._is_valid_plan():
            raise ValueError("Invalid plan: must contain each value of PromptComponent exactly once.")

    def _convert_to_enum(self, prompt_order: tuple):
        """Convert string names to PromptComponent enum values."""
        return tuple(PromptComponent(component) if isinstance(component, str) else component for component in prompt_order)
    ...
    def arrange_components(self, **kwargs) -> Dict[PromptComponent, Prompt]:
        ...
        if not set(kwargs.keys()) == component_strings:
            raise ValueError(f"Invalid components passed: {set(kwargs.keys())} but expected {PromptComponent}")
        
        user_prompt = PromptList([kwargs[component.value] for component in self.user_prompt_order])
        system_prompt = PromptList([kwargs[component.value] for component in self.system_prompt_order])
...
class PromptConstructorMixin:
    ...
    def agent_instructions_prompt(self) -> Prompt:
        ...
        if not hasattr(self, "_agent_instructions_prompt"):
            ...
            self._agent_instructions_prompt = applicable_prompts[0](text=self.agent.instruction)
    ...
    def construct_system_prompt(self) -> Prompt:
        ...
        """
        import warnings
        warnings.warn("This method is deprecated. Use get_prompts instead.", DeprecationWarning)
