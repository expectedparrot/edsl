...
class PerformanceDelta(Base):
    ...
    def _infer_metric_direction(self) -> Dict[str, bool]:
        ...
        if hasattr(self.baseline_sl, 'codebook') and self.baseline_sl.codebook:
            metric_names = [
                key for key in self.baseline_sl.codebook.keys()
                if key not in ('question', 'question_text', 'answer_a', 'answer_b', 'question_type')
            ]
        else:
            ...
            if len(self.baseline_sl) > 0:
                ...
                metric_names = [
                    key for key in first_scenario.keys()
                    if key not in ('question', 'question_text', 'answer_a', 'answer_b', 'question_type')
                ]
            else:
                metric_names = []
    ...
    def _calculate_deltas(self) -> Dict[str, Dict[str, int]]:
        ...
        for baseline_scenario, updated_scenario in zip(self.baseline_sl, self.updated_sl):
            ...
            for key in baseline_scenario.keys():
                ...
                if key in ('question', 'question_text', 'answer_a', 'answer_b', 'question_type'):
                    continue
    ...
    def plot_matrix(
        ...
        # Add legend
        legend_elements = [
            mpatches.Patch(facecolor='#22c55e', edgecolor='gray', label='Improved (+1)'),
            mpatches.Patch(facecolor='#ffffff', edgecolor='gray', label='Unchanged (0)'),
            mpatches.Patch(facecolor='#ef4444', edgecolor='gray', label='Worsened (-1)')
        ]
