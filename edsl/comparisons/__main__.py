...
def main():
    ...
    from .metrics.metric_definitions import (
        ExactMatch,
        CosineSimilarity,
        Overlap,
        SquaredDistance,
    ...
    try:
        custom_factory = (
            ComparisonFactory().add_comparison(ExactMatch()).add_comparison(Overlap())
        )
        print(
    ...
    agent_list = CandidateAgentList(
        sample_agents, ["Helpful", "Analytical", "Creative"]
    )
    ...
    optimizer = AgentOptimizer(
        survey=survey, gold_standard=gold_standard, starting_agents=agent_list
    )
    ...
    initial_results, initial_comparisons = optimizer.evaluate_initial_performance(
        verbose=False
    )
    print(f"✅ Evaluated {len(sample_agents)} agents against gold standard")
    print(
        f"Initial performance data collected for {len(initial_comparisons)} comparisons"
    )

    print("\n🎯 Identifying optimization targets...")
    target_agents = optimizer.identify_optimization_targets(
        selection_strategy="all", verbose=False
    )
    ...
    print("    edsl_agents = results.to_edsl_agent_list()")
