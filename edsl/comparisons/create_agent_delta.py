...
Please provide a structured analysis."""
        ...
        analysis_prompt = custom_analysis_prompt.format(digest=digest) if "{digest}" in custom_analysis_prompt else custom_analysis_prompt + f"\n\n{digest}"
...
Provide specific, concrete values rather than vague suggestions."""
    ...
    q_analysis = QuestionFreeText(
        question_name="persona_analysis",
        question_text=analysis_prompt
    )
    ...
    if agents is not None:
        ...
        if comparison_names != agent_names:
            ...
            if missing_in_comparisons:
                error_parts.append(f"Missing comparisons: {sorted(missing_in_comparisons)}")
