...
class PromptAdjust:
    ...
    def __init__(
        ...
        try:
            ...
            if len(personas) < agent_count:
                raise ValueError(f"Cannot access all agent personas: expected {agent_count}, got {len(personas)}")
        ...
        for question_name in self.gold_standard_dict.keys():
            try:
                ...
                if len(answers) < agent_count:
                    raise ValueError(f"Cannot access answers for question '{question_name}': expected {agent_count}, got {len(answers)}")
            except Exception as e:
                raise ValueError(f"Cannot access answers for question '{question_name}': {e}")
    ...
    def analyze_agent_gap(self, agent_name: str) -> str:
        ...
        for question_name, expected_answer in self.gold_standard_dict.items():
            ...
            match_status = "✓ CORRECT" if actual_answer == expected_answer else "✗ INCORRECT"
        ...
        analysis = f"Agent Name: {agent_name}\nOriginal Agent Persona: {persona}\n\nPERFORMANCE ANALYSIS:\n" + "\n".join(gaps)
        return analysis
    
    def generate_improved_personas(self, agent_names: list[str]) -> dict[str, dict[str, str]]:
        ...
        persona_question = QuestionFreeText(
            question_name="improved_persona",
            question_text=persona_question_text
        )
        
        conflict_question = QuestionFreeText(
            question_name="conflicts",
            question_text=conflict_question_text
        )
...
if __name__ == "__main__":
    ...
    with local_results_cache(job) as agent_results:
        ...
        adjuster = PromptAdjust(
            ...
            gold_standard_dict=gold_standard,
            survey=survey
        )
        ...
        for agent_name in all_agent_names:
            ...
            print(f"  • {agent_name}: {matches}/{total_questions} correct ({accuracy:.1%})")
        ...
        for agent_name in all_agent_names:
            ...
            improved_persona = agent_results.get("improved_persona", "No improvement generated")
        ...
        print(f"  • Direct gap-to-persona improvement (no complex instruction chains)")
        print(f"  • Conflict analysis to identify contradictions in persona changes")
        print(f"  • Batch processing for efficient multi-agent analysis")
        print(f"  • Performance measurement against gold standards")
        print(f"  • Rich table visualization of persona transformations")
        print(f"  • Clean, simple API with parallel LLM calls")
