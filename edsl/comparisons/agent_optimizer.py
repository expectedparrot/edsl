...
class AgentOptimizer:
    ...
    def _get_display_columns(self, results):
        ...
        comment_columns = [
            col for col in available_columns if col.startswith("comment.")
        ]
    ...
    def evaluate_initial_performance(self, verbose: bool = True) -> tuple:
        ...
        (
            ...
            self.initial_comparisons,
        ) = self.starting_agents.take_survey(
            self.survey,
        ...
        self.logger.info(
            f"Initial evaluation completed: {len(self.starting_agents.agents)} agents evaluated"
        )
    ...
    def identify_optimization_targets(
        ...
        if selection_strategy in ["top_percent", "top_count"] and metric is None:
            raise ValueError(
                f"Selection strategy '{selection_strategy}' requires 'metric' parameter"
            )

        if selection_strategy == "top_percent" and percentage is None:
            raise ValueError(
                "Selection strategy 'top_percent' requires 'percentage' parameter"
            )

        if selection_strategy == "top_count" and count is None:
            raise ValueError(
                "Selection strategy 'top_count' requires 'count' parameter"
            )
        ...
        if selection_strategy == "all":
            ...
            if verbose:
                self.console.print(
                    f"Selected ALL {len(targets.agents)} agents for optimization"
                )
        else:
            ...
            if verbose:
                strategy_desc = self._get_strategy_description(
                    selection_strategy, metric, percentage, count, question
                )
                self.console.print(
                    f"Selected {len(targets.agents)} agents using {strategy_desc}"
                )
                targets.show()

        self.logger.info(
            f"Selected {len(targets.agents)} agents for optimization using strategy: {selection_strategy}"
        )
    ...
    def _handle_user_confirmation(
        self, target_agents: CandidateAgentList, verbose: bool
    ) -> CandidateAgentList:
        ...
        self.console.print(
            "Each agent improvement involves multiple LLM calls and may take some time."
        )

        while True:
            try:
                user_input = input(
                    f"How many agents would you like to optimize? (0-{len(target_agents.agents)}): "
                ).strip()
                ...
                if 0 <= num_agents <= len(target_agents.agents):
                    ...
                    self.console.print(
                        f"[red]Please enter a number between 0 and {len(target_agents.agents)}[/red]"
                    )
            except ValueError:
        ...
        if num_agents < len(target_agents.agents):
            ...
            limited_info = (
                target_agents.info[:num_agents]
                if target_agents.info
                else [f"Agent {i+1}" for i in range(num_agents)]
            )
            limited_target_agents = CandidateAgentList(limited_agents, limited_info)
            if verbose:
                self.console.print(
                    f"[green]Will optimize the first {num_agents} agents[/green]"
                )
            return limited_target_agents
        else:
            if verbose:
                self.console.print(
                    f"[green]Will optimize all {num_agents} agents[/green]"
                )
    ...
    def _analyze_agent_performance(
        self, agent_name: str, adjuster: "PromptAdjust", verbose: bool
    ) -> dict:
    ...
    @log_method_calls
    def _apply_persona_improvement(
        self,
        ...
        if not improved_list.agents:
            if verbose:
                self.console.print(
                    f"[yellow]Warning: No improved agents generated for {agent_name}[/yellow]"
                )
            return []
    ...
    def _optimize_single_agent(
        ...
        analysis_results = self._analyze_agent_performance(
            agent_name, adjuster, verbose
        )
        ...
        if improved_persona and improved_persona.strip():
            improved_agents = self._apply_persona_improvement(
                agent, improved_persona, agent_name, verbose
            )
            ...
            if improved_agents:
                ...
                self.optimization_log.append(
                    {
                        ...
                        "improved_persona": improved_agents[0].persona
                        if improved_agents
                        else None,
                    }
                )
                ...
                if verbose:
                    self.console.print(
                        f"[green]Successfully optimized {agent_name}[/green]"
                    )
    ...
    @log_method_calls
    def _create_optimized_agents_list(
        self, optimized_agents: list, verbose: bool
    ) -> CandidateAgentList:
        ...
        if optimized_agents:
            try:
                ...
                if verbose:
                    self.console.print(
                        f"[red]❌ CandidateAgentList creation failed: {e}[/red]"
                    )
                return CandidateAgentList([], [])
        else:
            if verbose:
    ...
    def optimize_agents(
        ...
        for i, agent in enumerate(target_agents.agents):
            ...
            if verbose and improved_variants:
                self.console.print(
                    f"[green]Added {len(improved_variants)} improved agents. Total so far: {len(optimized_agents)}[/green]"
                )

        # Create and store final optimized agents list
        self.optimized_agents = self._create_optimized_agents_list(
            optimized_agents, verbose
        )
    ...
    def evaluate_final_performance(self, verbose: bool = True) -> tuple:
        ...
        self.logger.info(
            f"Final evaluation completed: {len(self.optimized_agents.agents)} agents evaluated"
        )
    ...
    def _show_enhanced_performance_table(self):
        ...
        enhanced_table = Table(
            title="Final Agent Performance with Perfect Question Rate", show_lines=True
        )
        enhanced_table.add_column("Agent", style="bold magenta")
        enhanced_table.add_column(
            "% Perfect Questions", style="green", justify="center"
        )
        ...
        for idx, (comp, agg_metrics) in enumerate(
            zip(self.final_comparisons, aggregated_metrics)
        ):
            # Calculate perfect questions count
            comp_dict = comp.compare()
            total_questions = len(comp_dict)
            perfect_questions = sum(
                1 for ac in comp_dict.values() if ac["exact_match"] == 1.0
            )
            perfect_fraction = (
                perfect_questions / total_questions if total_questions > 0 else 0
            )
            ...
            display_persona = (
                new_persona
                if len(new_persona) <= max_persona_len
                else new_persona[: max_persona_len - 3] + "..."
            )
    ...
    def optimize(
        ...
        try:
            ...
            if verbose:
                ...
                self.console.print(
                    f"• Initial agents: {summary['initial_agent_count']}"
                )
                self.console.print(
                    f"• Optimized agents: {summary['optimized_agent_count']}"
                )
                self.console.print(
                    f"• Improvement rate: {summary['improvement_rate']:.1%}"
                )
                if summary["average_perfect_questions"] is not None:
                    self.console.print(
                        f"• Average perfect questions: {summary['average_perfect_questions']:.1%}"
                    )
    ...
    def get_optimized_agents_summary(self) -> dict:
        ...
        """
        if not self.optimized_agents or not self.optimized_agents.agents:
            return {"agents": [], "agent_details": [], "count": 0}
            ...
            agent_details.append(
                {
                    ...
                    "optimization_log": self.optimization_log[i]
                    if i < len(self.optimization_log)
                    else None,
                }
            )
    ...
    def _get_optimization_overview(self):
        ...
        return {
            ...
            "successfully_optimized_agents": len(self.optimized_agents.agents)
            if self.optimized_agents
            else 0,
            "optimization_success_rate": len(self.optimized_agents.agents)
            / len(self.optimization_log)
            if self.optimization_log
            else 0,
            "total_questions": len(self.survey.questions),
            "gold_standard_coverage": len(self.gold_standard)
            ...
            if original_idx is not None and i < len(final_aggregated):
                ...
                for metric in initial_metrics:
                    ...
                    if isinstance(initial_val, (int, float)) and isinstance(
                        ...
                        if not (
                            initial_val != initial_val or final_val != final_val
                        ):  # Filter NaN
                            improvement = final_val - initial_val
                            percent_change = (
                                (improvement / initial_val * 100)
                                if initial_val != 0
                                else 0
                            )
                ...
                if valid_metrics > 0:
                    agent_improvement["overall_improvement"] = (
                        total_improvement / valid_metrics
                    )
            ...
            for metric in metric_names:
                ...
                for agent_imp in improvements["per_agent_improvements"]:
                    if metric in agent_imp["metric_improvements"]:
                        metric_improvements.append(
                            agent_imp["metric_improvements"][metric][
                ...
                if metric_improvements:
                    improvements["aggregate_improvements"][metric] = {
                        "mean_improvement": sum(metric_improvements)
                        / len(metric_improvements),
                        "median_improvement": sorted(metric_improvements)[
                            len(metric_improvements) // 2
                        ],
                        ...
                        "std_improvement": (
                            sum(
                                (
                                    x
                                    - sum(metric_improvements)
                                    / len(metric_improvements)
                                )
                                ** 2
                                for x in metric_improvements
                            )
                            / len(metric_improvements)
                        )
                        ** 0.5,
    ...
    def _analyze_convergence(self):
        ...
        convergence = {
            ...
            "pareto_percentage": len(self.final_comparisons.nondominated())
            / len(final_aggregated)
            * 100,
            "metric_convergence": {},
        ...
        for question in [q.question_name for q in self.survey.questions]:
            ...
            if initial_scores and final_scores:
                ...
                question_analysis[question] = {
                    ...
                    "difficulty_rank": 1
                    - initial_mean,  # Higher rank = more difficult initially
                    "optimization_effectiveness": (final_mean - initial_mean)
                    / (1 - initial_mean)
                    if initial_mean < 1
                    else 0,
                }
    ...
    def _analyze_agent_diversity(self):
        ...
        if self.initial_comparisons:
            ...
            if initial_aggregated:
                diversity["initial_diversity"] = self._calculate_population_diversity(
                    initial_aggregated
                )
        ...
        if self.final_comparisons:
            final_aggregated = self.final_comparisons._aggregated_metrics()
            if final_aggregated:
                diversity["final_diversity"] = self._calculate_population_diversity(
                    final_aggregated
                )
                ...
                if metric in diversity["final_diversity"]:
                    ...
                    diversity["diversity_change"][metric] = {
                        ...
                        "variance_change_percent": (
                            (final_var - initial_var) / initial_var * 100
                        )
                        if initial_var > 0
                        else 0,
                    }
    ...
    def _analyze_optimization_efficiency(self):
        ...
        if self.optimization_log:
            ...
            for log_entry in self.optimization_log:
                ...
                agent_total = sum(
                    len(suggestions) if isinstance(suggestions, list) else 1
            ...
            if agent_suggestion_counts:
                efficiency["suggestions_per_agent"] = {
                    ...
                    "median": sorted(agent_suggestion_counts)[
                        len(agent_suggestion_counts) // 2
                    ],
            ...
            if all_suggestions:
                ...
                efficiency["optimization_patterns"] = {
                    ...
                    "vocabulary_diversity": len(set(filtered_words))
                    / len(filtered_words)
                    if filtered_words
                    else 0,
                }
            ...
            for i, log_entry in enumerate(self.optimization_log):
                ...
                if original_idx is not None and i < len(final_aggregated):
                    ...
                    if (
                        ...
                        and not (initial_val != initial_val or final_val != final_val)
                    ):
                        initial_values.append(initial_val)
                        final_values.append(final_val)

            if (
                len(initial_values) >= 2
            ):  # Need at least 2 pairs for meaningful statistics
                differences = [f - i for f, i in zip(final_values, initial_values)]
                mean_diff = sum(differences) / len(differences)

                if len(differences) > 1:
                    var_diff = sum((d - mean_diff) ** 2 for d in differences) / (
                        len(differences) - 1
                    )
                    ...
                    t_stat = (
                        mean_diff / (std_diff / (len(differences) ** 0.5))
                        if std_diff > 0
                        else 0
                    )
                    ...
                    pooled_std = (
                        (
                            sum(
                                (i - sum(initial_values) / len(initial_values)) ** 2
                                for i in initial_values
                            )
                            + sum(
                                (f - sum(final_values) / len(final_values)) ** 2
                                for f in final_values
                            )
                        )
                        / (len(initial_values) + len(final_values) - 2)
                    ) ** 0.5
    ...
    def _generate_summary(self) -> dict:
        ...
        summary = {
            ...
            "optimized_agent_count": len(self.optimized_agents.agents)
            if self.optimized_agents
            else 0,
        ...
        if summary["initial_agent_count"] > 0:
            summary["improvement_rate"] = (
                summary["optimized_agent_count"] / summary["initial_agent_count"]
            )
