from edsl.comparisons.result_pair_comparison.result_pair_comparison import (
...
class EvaluateAgentsAgainstGold(UserDict):
    ...
    def __init__(self, result_pair_comparison_list: List["ResultPairComparison"]):
        ...
        for rpc in self.rpc_list:
            ...
            if rpc.result_A.agent.base_name != agent_base_name:
                raise ValueError(
                    f"""All agents in the list must be the base same. 
        ...
        for question_name, metrics in best_metrics.items():
            d[question_name] = {
                ...
                "pareto_frontier": pareto_frontier.data[question_name],
            }
        super().__init__(d)
    ...
    def _summary_repr(self, MAX_QUESTIONS: int = 10, MAX_METRICS: int = 10) -> str:
        ...
        output.append(
            f"    num_agents={len(self.rpc_list)},\n", style=RICH_STYLES["default"]
        )
        ...
        for i, (question_name, question_data) in enumerate(
            list(self.items())[:num_to_show]
        ):
            # Show question name
    ...
    def _best_agent_by_score(self) -> int:
        ...
        index = next(
            index for index, score in self.scores.items() if score == max_score
        )
