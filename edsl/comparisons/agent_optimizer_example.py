...
def main():
    ...
    optimizer = AgentOptimizer(
        survey=survey, gold_standard=gold_standard, starting_agents=starting_agents
    )
    ...
    if len(results) > 0:
        ...
        for i, log_entry in enumerate(results.optimization_log):
            ...
            print(
                f"   Questions improved: {list(log_entry['suggestions_per_question'].keys())}"
            )
    ...
    print("• Optimization log: results.optimization_log")
    print("• Performance comparisons: results.final_comparisons")
    ...
    if len(edsl_agents) > 0:
        ...
        print("  Ready to run: confidence_survey.by(edsl_agents).run()")
        print("  The optimized personas will be used automatically!")
