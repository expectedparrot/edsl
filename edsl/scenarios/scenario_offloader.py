...
class ScenarioOffloader:
    ...
    def requires_offloading(self) -> bool:
        ...
        if (
            "base64_string" in self.scenario
            and isinstance(self.scenario.get("base64_string"), str)
            and self.scenario.get("base64_string") != "offloaded"
        ):
            return True
        ...
        for key, value in self.scenario.items():
            if isinstance(value, FileStore):
                ...
                if (
                    "base64_string" in file_store_dict
                    and file_store_dict["base64_string"] != "offloaded"
                ):
                    return True
            elif (
                isinstance(value, dict)
                and "base64_string" in value
                and value["base64_string"] != "offloaded"
            ):
                return True
