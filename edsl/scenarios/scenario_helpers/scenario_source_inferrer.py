...
class ScenarioSourceInferrer:
    ...
    def infer_and_create(
        source: Any, field_name: Optional[str] = None, verbose: bool = False, **kwargs
    ) -> "ScenarioList":
        """
        ...
        if isinstance(source, (str, Path)):
            return ScenarioSourceInferrer._handle_string_or_path(
                source, field_name, verbose, **kwargs
            )
    ...
    @staticmethod
    def _handle_list(
        source: list, field_name: Optional[str] = None, **kwargs
    ) -> "ScenarioList":
        ...
        use_indexes = kwargs.pop("use_indexes", False)
        return ScenarioSource.from_source(
            "list", field_name, source, use_indexes, **kwargs
        )
        ...
        **kwargs,
        ...
        if os.path.exists(source_str):
            if os.path.isdir(source_str):
                return ScenarioSourceInferrer._handle_directory(
                    source_str, verbose, **kwargs
                )
            elif os.path.isfile(source_str):
                return ScenarioSourceInferrer._handle_file(
                    source_str, verbose, **kwargs
                )

        # If file doesn't exist, raise an error
        raise ScenarioError(f"File or directory not found: {source_str}")
    ...
    def _handle_url(url: str, verbose: bool = False, **kwargs) -> "ScenarioList":
        ...
        if "wikipedia.org" in url_lower:
            ...
            return ScenarioSource.from_source(
                "wikipedia", url, table_index, header, **kwargs
            )

        # Check for Google Sheets
        if (
            "docs.google.com/spreadsheets" in url_lower
            or "sheets.google.com" in url_lower
        ):
            if verbose:
        ...
        if url_lower.endswith(".csv"):
            ...
            sheet_name = kwargs.pop("sheet_name", None)
            return ScenarioSource.from_source(
                "excel", url, sheet_name=sheet_name, **kwargs
            )
    ...
    def _handle_directory(
        directory: str, verbose: bool = False, **kwargs
    ) -> "ScenarioList":
    ...
    def _handle_file(file_path: str, verbose: bool = False, **kwargs) -> "ScenarioList":
        ...
        # Excel files
            ...
            sheet_name = kwargs.pop("sheet_name", None)
            return ScenarioSource.from_source(
                "excel", file_path, sheet_name=sheet_name, **kwargs
            )
        ...
        # SQLite database
            ...
            return ScenarioSource.from_source(
                "sqlite", file_path, table, fields, **kwargs
            )
        ...
        # Stata files
            ...
            return ScenarioSource.from_source(
                "dta", file_path, include_metadata, **kwargs
            )
        ...
        # LaTeX files
            ...
            return ScenarioSource.from_source(
                "latex", file_path, table_index, has_header, **kwargs
            )
