...
class TestExaLoader:
    ...
    def test_enrichment_validation(self):
        ...
        with patch.dict(os.environ, {"EXA_API_KEY": "test-key"}):
            with patch("edsl.scenarios.exa.loader.Exa"):
                ...
                with pytest.raises(
                    ValueError, match="Each enrichment must be a dictionary"
                ):
                    from_exa("test query", enrichments=["invalid_enrichment"])

                # Test missing required keys
                with pytest.raises(
                    ValueError, match="Each enrichment must have a 'description' key"
                ):
                    from_exa("test query", enrichments=[{"format": "text"}])

                with pytest.raises(
                    ValueError, match="Each enrichment must have a 'format' key"
                ):
    ...
    def test_webset_parameters_construction(self):
        ...
        with patch.dict(os.environ, {"EXA_API_KEY": "test-key"}):
            with patch("edsl.scenarios.exa.loader.Exa", mock_exa_class):
                with patch(
                    "edsl.scenarios.exa.loader.CreateWebsetParameters",
                    mock_params_class,
                ):
                    from_exa(
                        query="test query",
    ...
    def test_runtime_error_on_api_failure(self):
        ...
        with patch.dict(os.environ, {"EXA_API_KEY": "test-key"}):
            with patch("edsl.scenarios.exa.loader.Exa", mock_exa_class):
                with patch("edsl.scenarios.exa.loader.CreateWebsetParameters"):
                    with pytest.raises(
                        RuntimeError, match="Failed to create EXA webset: API Error"
                    ):
                        from_exa("test query")
