...
def example_sales_leaders():
    ...
    print(f"First scenario keys: {list(scenarios[0].keys()) if scenarios else 'No scenarios'}")
    return scenarios
...
def example_from_webset():
    ...
    try:
        ...
        print(f"Loaded ScenarioList with {len(scenarios)} scenarios from webset {webset_id}")
        return scenarios
