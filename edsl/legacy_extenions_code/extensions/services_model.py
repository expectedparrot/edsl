...
class ServicesRegistry(UserDict):
    ...
    def __init__(
        self,
        service_to_ep_info: Optional[Dict[str, Any]] = None,
    ...
    def update(self):
        ...
        if hasattr(self, "registry_uuid"):
            ...
            warnings.warn(
                "Using registry_uuid to update registry; this is a temporary work-around"
            )
    ...
    def update_service(self, service: ServiceDefinition):
        ...
        if service.name in self.service_to_ep_info:
            ...
            service.update(
                service_uuid
            )  # self.service_to_ep_info[service.name] = service.push()
        else:
...
if __name__ == "__main__":
    if False:
        ...
        info = services_registry.push(
            alias="ep-services-experimental",
            description="EP-Services Experimental",
            visibility="public",
        )
        print(info)
    else:
