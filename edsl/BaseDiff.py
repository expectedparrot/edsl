...
class BaseDiff:
    def __init__(self, obj1: Any, obj2: Any, added=None, removed=None, modified=None, level = 0):
    ...
    def _find_modified(self) -> Dict[Any, Tuple[Any, Any, str]]:
        ...
        for k in self._dict1:
            if k in self._dict2 and self._dict1[k] != self._dict2[k]:
                ...
                elif isinstance(self._dict1[k], dict) and isinstance(self._dict2[k], dict):
                    diff = self._diff_dicts(self._dict1[k], self._dict2[k])
                    modified[k] = (self._dict1[k], self._dict2[k], diff)
                elif isinstance(self._dict1[k], list) and isinstance(self._dict2[k], list):
                    d1 = dict(zip(range(len(self._dict1[k])), self._dict1[k]))
                    d2 = dict(zip(range(len(self._dict2[k])), self._dict2[k]))
                    diff = BaseDiff(DummyObject(d1), DummyObject(d2), level = self.level + 1)
                    modified[k] = (self._dict1[k], self._dict2[k], diff)                         
                else:
    ...
    def __repr__(self):
        return (f"BaseDiff(obj1={self.obj1!r}, obj2={self.obj2!r}, added={self.added!r}, "
