...
class QuestionCheckboxGrid(Question):
    ...
    def instructions(self):
        return textwrap.dedent(
            """\
        You are being asked the following question in the form of a matrix: 
        {{question_text}}
        The matrix rows are 
        ...
        {"answer": <[put list of lists representing column option(s) selected for each row here]>, 
    ...
    def construct_question_data_model(cls) -> Type[BaseModel]:
        class LocalQuestionData(QuestionData):
            """Pydantic data model for QuestionCheckboxGrid"""
...
if __name__ == "__main__":
    ...
    print("Now validating checkbox grid questions")

    q = QuestionCheckboxGrid(
        question_text="Indicate which days of the week you would eat each item.",
        row_options=["Pizza", "Ice Cream"],
        column_options=["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
        required=True,
    )
    print(q.get_prompt())
