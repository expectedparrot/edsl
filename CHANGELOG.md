...
- `Results` now include a field `generated_tokens` with the tokens that were generated by the model.
...
- Optional `Question` parameter `include_comments = False` allows you to turn off the comment field that is automatically added to all question types other than `QuestionFreeText`. 
