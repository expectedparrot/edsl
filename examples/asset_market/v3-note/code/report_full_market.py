...
def build(path):
    ...
    calls = [
        c for c in all_calls if isinstance(c["result"]["answer"].get("decision"), dict)
    ]
    incomplete_calls = [
        c
        for c in all_calls
        if not isinstance(c["result"]["answer"].get("decision"), dict)
    ]
    ...
    with sqlite3.connect(f"file:{path}/workflow.sqlite?mode=ro", uri=True) as db:
        ...
        submitted = {
            item: json.loads(answers)
            for item, answers in db.execute(
                "select work_item_id,answers from workflow_submissions"
            )
        }
    assert attempts.get("succeeded") == 390
    ...
    assert len(views) == 360
    assert all(r["value"] == views[r["execution_id"]] for r in reads)
    questions = {
    ...
    for call in calls:
        ...
        result = call["result"]
        assert submitted[call["work_item_id"]] == result["answer"]
        rendered = (
    ...
    summary = {
        ...
        "incomplete_result_records": len(incomplete_calls),
        "audit": {
            "passed": True,
            "orders": dict(writes),
            "attempts": attempts,
    ...
    if batch_file.exists():
        ...
        started_batches = [row for row in batches if row["event"] == "started"]
        batched_ids = {item for row in started_batches for item in row["work_item_ids"]}
        batched_decisions = sum(c["work_item_id"] in batched_ids for c in calls)
        timings = [
            row["elapsed_seconds"]
        ...
        summary["execution"] = {
            "initial_serial_decisions": len(calls) - batched_decisions,
            "batched_decisions": batched_decisions,
            "edsl_jobs_completed": len(completed_batches),
            "edsl_jobs_started": len(started_batches),
            "batches_without_completion_record": len(started_batches)
