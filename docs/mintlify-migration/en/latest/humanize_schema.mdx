...
<ParamField path="questions" type="dict" required={false}>
  ...
  <Expandable title="properties">
    ...
    <ParamField path="questions.question_name.format" type="object" required={false}>
      Display format: `"radio"` (list of radio buttons) or `"dropdown"` (single `<select>`). Default is `"radio"` when omitted. Use `{"type": "radio"}` or `{"type": "dropdown"}`.
      
      **Supported question types:** `likert_five`, `linear_scale`, `multiple_choice`, `yes_no`. Other question types do not accept `format`.
    </ParamField>
  </Expandable>
</ParamField>
