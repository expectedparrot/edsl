    ...
    <ParamField path="checklist" type="dict" required={false}>
      ...
      <Expandable title="properties">
        <ParamField path="items" type="array" default="[]">
          ...
          - `id` (string, required) — a unique identifier, 1–64 characters, restricted to letters, digits, underscores, and hyphens (`^[A-Za-z0-9_\-]+$`). Item ids must be unique within the checklist.
