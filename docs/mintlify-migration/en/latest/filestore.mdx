---
title: "File Store"
description: "*FileStore* is a module for storing and sharing data files at Expected Parrot. It allows you to post and retrieve files of various types to use in EDSL surveys, such as survey data, PDFs, CSVs, docs or images. It can also be used to create Scenario objects for questions or traits for Agent objects from data files stored on the platform."
---
...
The examples below are also available in a [notebook at Expected Parrot](https://www.expectedparrot.com/content/RobinHorton/my-example-filestore-notebook).
...
2. Call the push method to post the file at Expected Parrot. You can optionally pass the following parameters:
...
The push method returns a dictionary with the following keys and values (this is the same for any object posted to Expected Parrot):
...
- url: the URL of the file at Expected Parrot
- uuid: the UUID of the file at Expected Parrot
...
Here we create an example CSV and then post it to Expected Parrot using FileStore and retrieve it. Then we use the retrieved file to construct scenarios for questions (you can skip the step to create a CSV and replace with your own file).
...
Here we post the file to Expected Parrot and inspect the details:
...
csv_info # display the URL and uuid of the stored file for retrieving it later
```
...
png_info # display the URL and uuid of the stored file for retrieving it later
```
...
Here we download an example PDF from the internet, post and retrieve it from Expected Parrot using FileStore and then convert it into a ScenarioList object with the from_pdf() method. The default keys are filename, page, text, which can be modified with the rename method.
...
Pull a FileStore object from Expected Parrot.
...
Push the object to Expected Parrot. :param description: The description of the object to push. :param visibility: The visibility of the object to push.
