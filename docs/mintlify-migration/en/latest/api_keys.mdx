...
For instructions on using your Expected Parrot key only for accessing the platform, please see [this section](/en/latest/coop) of the documentation.
...
This method provides a secure way to store your keys and the most flexibility in choosing how to use them. It requires an Expected Parrot account ([log in/sign up here](https://www.expectedparrot.com/login)) and works with remote-inference.

*If you are only running surveys locally, see method 2. below for instructions on storing keys on your computer.*

To use this method, go to your [Settings](http://www.expectedparrot.com/home/settings) page and enable the **Run surveys remotely** toggle under Remote Execution:

<Frame>
![Settings page showing Remote Execution and Remote Error Logging toggles](/images/en/latest/home-settings.png)
</Frame>

Then go to your [Keys](http://www.expectedparrot.com/home/keys) page and choose whether to add any of your own keys. Use the **Grant access** icon to share a key with other users without sharing the key directly. Use the **Edit** icon to modify settings for any of your keys, or the **Delete** icon to remove them.

You can review the current prioritization of your keys at the **Key priority** tab at any time. When you run surveys remotely, your Expected Parrot key is used by default with any models that you have not provided keys for.

<Frame>
![Keys page showing LLM Keys and key management options](/images/en/latest/home-keys.png)
</Frame>

### 2. Store keys in a local file

This method allows you to store keys in a private file on your computer and make them available for any surveys that you run, locally or remotely. It also allows you to post local content to Expected Parrot.
...
```bash expandable
EXPECTED_PARROT_API_KEY = 'your_key_here' # This key is required for remote inference and interacting with Expected Parrot
...
<Frame>
![Keys page showing Expected Parrot API Key and local .env configuration](/images/en/latest/home-ep-key.png)
</Frame>

This key allows you to access all available models at once and use remote-inference to run surveys at the Expected Parrot server. It also allows you to post content to the platform, and to interact with other content that is public or shared with you.
