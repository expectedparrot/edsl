...
We select models to use with a survey by creating `Model` objects for them (if no model is specified, gpt-5.2 is currently used by default):
...
We can post any EDSL object to Expected Parrot by calling the `push()` method on it, including this notebook:
