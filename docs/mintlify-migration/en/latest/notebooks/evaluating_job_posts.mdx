...
- Create a [Expected Parrot account](https://www.expectedparrot.com/login) and activate [remote inference](/en/latest/remote_inference) to run surveys at the Expected Parrot server. You can use an Expected Parrot API key or your own [API Keys](/en/latest/api_keys) for language models that you want to use.
...
We can post any objects to Expected Parrot, including this notebook. Objects can be updated or modified at your Expected Parrot account, and shared with others or stored privately (default visibility is *unlisted*):
