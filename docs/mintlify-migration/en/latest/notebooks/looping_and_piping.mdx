...
Expected Parrot is a platform for posting and sharing AI-based research. It is fully integrated with EDSL and free to use. Learn more about [how it works](/en/latest/coop) or create an account: [https://www.expectedparrot.com/login](https://www.expectedparrot.com/login).

In the examples above, results generated using [remote inference](/en/latest/remote_inference) (run at the Expected Parrot server) were automatically posted to Expected Parrot (see links to results).

Here we show how to manually post any local content to Expected Parrot, such as this notebook:
