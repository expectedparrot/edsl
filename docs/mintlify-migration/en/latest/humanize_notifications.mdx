...
`create_callback` sends emails to respondents in your agent list. Two respondent event types are supported:
...
Use `create_transcript_callback` to automatically email each respondent a copy of their responses each time they submit. This uses the built-in `respondent_transcript` template.
