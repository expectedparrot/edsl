...
description: "Remote caching automatically stores responses from language models at the Expected Parrot server, and retrieves stored responses to questions that have already been run. This makes it free and instant to reproduce results from any survey that has been run remotely."
---
...
Remote caching is automatically activated with remote inference. To activate remote inference, navigate to the [Settings](https://www.expectedparrot.com/home/settings) page of your account and enable the **Run surveys remotely** toggle under Remote Execution. Learn more about how remote inference works at the [Remote Inference](/en/latest/remote_inference) section.

When you run a survey remotely at the Expected Parrot server your results are automatically cached. You can access them programmatically from your workspace (see examples of methods below). To turn off remote caching, navigate to the [Settings](https://www.expectedparrot.com/home/settings) page and disable the **Use remote cache** toggle.
...
- **Sharing & reproducibility:** A new cache is automatically attached to each results object, which can be posted and shared with other users at Expected Parrot.
...
Results that were generated at the Expected Parrot server will show a verified checkmark ✓ on the platform. If you rerun a survey with verified results you will retrieve stored responses.
</Accordion>
...
No, the universal remote cache is only available when running a survey remotely.
</Accordion>
...
Your remote cache entries include all of the responses to questions that you have run remotely, whether generated fresh or retrieved from the universal remote cache.

For example, here we run a survey with remote caching activated, and pass a description to readily identify the job at Expected Parrot:
...
When you share a results object (e.g., post it publicly at Expected Parrot or share it privately with other users) the cache attached to it is automatically shared with it. This can be useful if you want to share a specific historic cache for a survey or project (e.g., to allow other users to reproduce your results). You can inspect the cache for a results object by calling the cache property on a results object.
...
Create a survey object on Expected Parrot, then create a project from the survey.
...
Retrieve and return the EDSL settings stored on Expected Parrot. If no response is received within 5 seconds, return an empty dict.
...
Fetch a dict of rate limit config vars from Expected Parrot.
...
Fetch a list of working models from Expected Parrot.
...
Get a project from Expected Parrot.
