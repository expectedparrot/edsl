...
This can be useful where a model is not needed for part of a survey. 
The question type lets us define a function for evaluating scenarios and agent traits, which is passed as a parameter func in the following format:
...
An example of this can be seen in [this notebook](https://docs.expectedparrot.com/en/latest/notebooks/random_numbers.html), where we give agents different instructions for generating random numbers and then use a function to identify whether the responses are identical.
