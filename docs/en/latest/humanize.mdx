...
Use `humanize_schema={"survey": {"navigation": {"back_button": True}}}` to request Back navigation. Question-level `navigation.allow_back_past` controls boundaries. See [Back navigation](/en/latest/humanize_navigation) for a complete example, CLI controls, local sessions, and availability requirements.
