---
title: Surveys
description: "A Survey is a collection of [Questions](/en/latest/questions) administered to one or more AI [Agents](/en/latest/agents) and [Language Models](/en/latest/language_models). Questions run asynchronously by default, or according to rules like skip/stop logic, with optional memory of prior answers."
---

Surveys collect data, generate content, or perform other tasks. Results are stored in a Results object for analysis, with responses cached locally or remotely for reuse. See the [Results](/en/latest/results) and [Remote Caching](/en/latest/remote_caching) sections for details.

Surveys can also contain display-only, routable steps using [`SurveyMessage`](/en/latest/survey_messages), for example introductions, eligibility notices, section transitions, and completion pages.
...
<Frame>
![Survey Flow Diagram with Skip Rule](/images/en/latest/show_flow.png)
</Frame>
...
<Frame>
![Survey Flow Diagram with Agent Information](/images/en/latest/show_flow_agent.png)
</Frame>
...
Send the survey to Expected Parrot.
