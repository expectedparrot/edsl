...
q_satisfied = QuestionMultipleChoice(
    ...
    question_text = "How satisfied are you with the following experience with our company: {{ scenario.experience }}",
    question_options = [
        "Extremely satisfied",
...
If no model is specified when running a survey, the default model gpt-5.2 is used (as above when we generated personas).
</Note>
...
Expected Parrot is a platform for creating, storing and sharing LLM-based research. It is fully integrated with EDSL, allowing you to access objects from your workspace or your account. [Learn more about creating an account and using the platform](/en/latest/coop).
