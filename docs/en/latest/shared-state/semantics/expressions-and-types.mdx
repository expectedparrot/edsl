...
description: "Shared-state language specification: expressions and types."
---
...
## Implemented accumulator expressions

The current interpreter also provides lexical `let`, sequence `fold`, bounded
`iterate`, `take`, `exp`, and stable `logsumexp` expressions. Their operands and
bodies are serialized in the existing expression representation. See
[Primitive examples and validation](/en/latest/shared-state/algorithms-and-reference)
for their operational contracts, scope rules, budget boundaries, and examples.
The iteration cap does not establish the specification's global resource-bound
requirements or a complete static type system. Older interpreters that do not
recognize these operators must reject the definitions.

## Implemented record and scope checks

`T.record(fields, allow_extra=False)` declares required named members. Optional
member types admit null, not omission; `allow_extra=True` permits additional
finite JSON values. Initial values, inputs, and proposed state are checked
recursively. Machine validation checks lexical scope in all expression contexts
and rejects literal lookups of undeclared members on known closed records,
including unselected branches. Failures include the machine and expression path.

These checks are a conservative subset of the type judgment described above.
The interpreter checks known operand compatibility, preserves compatible numeric
and nested-container branch shapes, and uses explicitly declared
`accumulator_type`/`state_type` invariants in loops. Seeds and each carried value
are also validated at runtime. Undeclared accumulator types remain unknown.
Unlike the full judgment above, unrelated branches currently widen to `any`;
null safety, dynamic lookup proofs, and complete reducer inference are not
implemented. These are implementation boundaries, not full conformance claims. See
[Records and early validation](/en/latest/shared-state/machines#records-and-early-validation)
