...
description: "Expected Parrot provides a free platform for storing, sharing and collaborating on AI-based research, fully integrated with EDSL via the Coop module."
sidebarTitle: "Expected Parrot"
---


Expected Parrot provides a free platform for creating, storing and sharing AI-based research, and validating it with human respondents. It is fully integrated with EDSL via the *Coop* module, allowing you to post, store and retrieve any objects that you've created with EDSL, together with data, notebooks and other project content.

You can also explore public content and collaborate on projects privately with other users:

<Frame>
<img src="/images/en/latest/coop_1.png" alt="Expected Parrot page view" />
</Frame>

Your Expected Parrot account provides access to special features for running EDSL surveys and storing results remotely at the Expected Parrot server, and launching hybrid human-AI inference jobs interactively. Learn more about these features in the [Remote Inference](/en/latest/remote_inference), [Remote Caching](/en/latest/remote_caching) and [Survey Builder](/en/latest/survey_builder) sections of the documentation.
...
### 3. Post objects to Expected Parrot
...
See details on methods for uploading, downloading, updating and deleting content at your account below.

### 4. Explore content

Content that you have created with remote inference or posted to Expected Parrot is available at your [Content](https://www.expectedparrot.com/content) page.

You can access public content at the [Explore](https://www.expectedparrot.com/login) tab, and copy code and examples to modify or rerun at your workspace.
...
You can also post, update, download and delete any objects using the `Coop` module and methods (see examples below).

### Posting

There are 2 methods for uploading/posting an object from your workspace to your account:

1. Call the *push()* method on the object directly.
2. Call the *create()* method on a `Coop` client object and pass it the object.

You can optionally pass a description, a convenient alias for the Expected Parrot URL and a *visibility* status (public, private or unlisted (default)) when you create an object. These can be changed at any time.
...
This will return the same information about the object as the direct method shown above (with a unique uuid and URL for viewing the object at the Expected Parrot platform).

### Updating

There are 3 methods for updating/editing an object to Expected Parrot:

1. Edit the object at the Expected Parrot platform.
...
**At the Expected Parrot platform**

You can manually update the description or visibility of an object at the Expected Parrot platform. Navigate to the [Explore](https://www.expectedparrot.com/login) page and open the page view for an object. You can select options to change the **visibility** of the object (*public*, *private* or *unlisted*) or to **edit** the description.
...
There are a variety of methods for replicating or downloading an object at Expected Parrot:

1. Selecting options to download or copy code at the Expected Parrot platform
2. Calling the *pull()* method on the class of the object
3. Calling the *get()* method on a `Coop` client object

**Copy code at the Expected Parrot platform**

The platform provides copyable code for downloading or reconstructing an object that has been posted:

<Steps>
  <Step>
  Navigate to the [Explore](https://www.expectedparrot.com/login) page and select an object
  </Step>
  <Step>
...
There are 3 methods for deleting an object from your account:

1. Selecting options to delete at the web app
2. Calling the *delete()* method on the class of the object
3. Calling the *delete()* method on a `Coop` client object

**At the Expected Parrot platform**

You can manually delete objects at the Expected Parrot platform:

<Steps>
  <Step>
  Navigate to [Explore](https://www.expectedparrot.com/login) and open an object’s page view (see image above for *Uploading* content)
  </Step>
  <Step>
  Select the option to **delete** the object:

<Frame>
<img src="/images/en/latest/coop_5.png" alt="Expected Parrot page view" />
</Frame>
  </Step>
</Steps>
...
You can create a project at the Expected Parrot platform by selecting the option to **Create a project** at the top right of the page.
