---
title: "Credits"
description: "Credits are required in order to run surveys with models using your Expected Parrot API key. They can be purchased at your account (see details below). Credits are deducted based on the applicable rates published on the Expected Parrot model pricing page. These are the rates Expected Parrot uses to calculate credit consumption and may differ from rates charged by the underlying service provider."
---

Credits must be purchased in advance and are consumed when surveys are run. If your balance is insufficient, you will be prompted to purchase more credits. Details on credit consumption are provided below.

<Note>
*Note:* Credits are *not* required to run surveys with your own keys from service providers or to post and share content at [Expected Parrot](https://www.expectedparrot.com/login). When using your own keys, cost estimates may use the prices listed on the model pricing page, but your actual charges are determined by the applicable service provider and may differ from those estimates.
</Note>

## Free credits

Your Expected Parrot account comes with free credits that you can use to run surveys with your API key.

**Are you using EDSL for a research project?**

Send an email to [*info@expectedparrot.com*](mailto:info@expectedparrot.com) to request additional free credits!

## Purchasing credits

To purchase credits, navigate to the [Credits](https://www.expectedparrot.com/home/credits) page of your account and enter the number of credits that you would like to purchase (1 USD buys 100 credits; the minimum purchase amount is 1 USD):

<Frame>
![Purchase credits](/images/en/latest/credits_1.png)
</Frame>
...
When you run a survey with your Expected Parrot API key, the number of credits consumed (and deducted from your balance) is displayed at the [Jobs](https://www.expectedparrot.com/home/remote-inference) page of your account. This number is equal to the sum of the cost in credits of each response in the results.

The cost in credits of a response is calculated as follows:

- The number of input tokens is multiplied by the applicable input token rate published on the Expected Parrot model pricing page.
- The number of output tokens is multiplied by the applicable output token rate published on the Expected Parrot model pricing page.
...
- **raw_model_response.&lt;question_name&gt;_cost**: The cost in USD calculated for the API call using the applicable Expected Parrot pricing rates. (In the example above, this is USD 0.00049.)
...
The total cost of the response is calculated using the applicable rates listed on the Expected Parrot model pricing page (at the time of writing, USD 2.50 per 1M tokens for input and USD 10.00 per 1M tokens for output):
...
The total cost of the response is calculated using the applicable rates listed on the Expected Parrot model pricing page (at the time of writing, USD 0.08 per 1M tokens for input and USD 0.30 per 1M tokens for output):
...
This translates to a total of 0.06 credits consumed for the survey. We can see this number of credits consumed at the remote inference page of our Expected Parrot account:

<Frame>
![Expected Parrot remote inference jobs page](/images/en/latest/credits_2.png)
</Frame>
...
<Frame>
![Expected Parrot remote inference jobs page](/images/en/latest/credits_3.png)
</Frame>


## Token rates

Model token rates used to calculate costs can be viewed on the [model pricing page](https://www.expectedparrot.com/getting-started/coop-pricing). For jobs run using an Expected Parrot API key, the rates published on that page are the rates Expected Parrot uses to calculate credit consumption.

Expected Parrot reviews pricing information from service providers regularly, but our published rates do not necessarily reproduce every provider-specific pricing mechanism, including discounts, promotions, time-based pricing, caching discounts, batch pricing, volume pricing, service tiers, or similar adjustments. Published Expected Parrot rates may therefore differ from the rates a service provider would charge for the same request made directly or using your own API key.

Provider pricing may change without notice, and Expected Parrot rates may not be updated at the same time. If a current price is unavailable, a default or fallback price may be used as described on the model pricing page. If you believe a published price is missing or out of date, please submit a report using the form on that page.
...
Here we create a survey and agent, select a model and combine them to create a job. Then we call the above-mentioned methods for estimating costs and show the underlying calculations. The steps below can also be accessed as a notebook at the Expected Parrot platform ([notebook view](https://www.expectedparrot.com/content/c379241a-7039-4505-8d42-4c909a54c6e0)).
...
3. Apply the token rates for the model and inference service.
    - Find the model and inference service for the question in the [model pricing page](https://www.expectedparrot.com/getting-started/coop-pricing):
...
| What color is `{{ answer }}`? | You are answering questions as if you were a human. Do not break character. Your traits: `{‘persona’: ‘You are a botanist on Cape Cod.’}` |
...
Please send an email to [*info@expectedparrot.com*](mailto:info@expectedparrot.com) if you have any questions about credits or refunds, or need assistance with your account.
