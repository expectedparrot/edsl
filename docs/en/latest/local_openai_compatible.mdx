...
Use `HostedModel` when constructing one hosted model directly in Python:

```python
from edsl import HostedModel, QuestionFreeText
...
question = QuestionFreeText(
    question_name="hello",
    question_text="Reply with exactly: hello",
)
results = question.by(model).run(disable_remote_inference=True)
```

`HostedModel` is a convenience factory for `Model(...,
service_name="openai_compatible")`. It returns a regular EDSL language model,
so model execution, error reporting, serialization, and client cleanup follow
the standard OpenAI-compatible service path. Use `api_key_env` to name the
credential environment variable; do not put the credential itself in a
serialized model.
