...
The survey's progress indicator sits above the question. It renders either as a bar or
as a row of step markers, depending on how the survey is configured; if progress is
hidden, nothing is rendered and there is nothing to style. Both renderings carry
`edsl-progress` on their outer element, so rules meant for progress in general can hang
off that one class.
...
| `edsl-progress-fill` | Filled portion | See the note below before setting `width` |
| `edsl-progress-label` | Percentage text under the bar | Absent when the label is turned off |

The fill spans the full width of the track and is slid left by a transform to reveal
only the completed portion, so setting `width` on it will not shorten it — style its
`background`, `border-radius`, or `transition` instead and leave the geometry alone.
...
<ol class="edsl-progress edsl-progress-steps">
  <li class="edsl-progress-step edsl-progress-step-complete">
    <div>
      <!-- first step's leading connector is hidden, not removed -->
      <span class="edsl-progress-connector"></span>
  ...
  <li class="edsl-progress-step edsl-progress-step-upcoming">
    <div>
      ...
      <!-- last step's trailing connector is hidden, not removed -->
      <span class="edsl-progress-connector"></span>
    </div>
...
Each step's status class is on the `<li>`, so style the pieces through it — that is how
you give the current step a different marker color from the finished ones:
...
.edsl-progress-step-complete .edsl-progress-connector { background: #b91c1c; }
```

Markers come in two shapes — numbered circles and small dots — set by the survey's
configuration rather than by CSS. Rules that assume a size (a fixed `width`, say) will
look wrong under the other shape, so if you change one, check the survey renders with
