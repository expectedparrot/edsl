...
| `edsl-logo` | The logo `<img>` inside `edsl-survey-banner` | 48px tall by default, width following the image. Resize with `height`; see below |
...
The logo is 48px tall by default, and its width follows the image's proportions. To resize
...
For a colored strip behind the logo, style the banner itself:
...
The banner sits inside the survey's page padding, so its background does not reach the
edges of the screen.
...
**Style markers and labels through their step.** Every `<li>` says whether its step is
...
`.edsl-progress-connector-complete`. Reaching for `.edsl-progress-step-complete
.edsl-progress-connector` instead leaves that first half gray, and the line stops short
...
**To reach a single segment, combine a step class with a connector class.** Include the
position class as well, since a step's two halves are otherwise identical. The rule below
