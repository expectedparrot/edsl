...
<ParamField path="survey" type="dict" required={false}>
  ...
  <Expandable title="properties">
    <ParamField path="survey.navigation" type="dict" required={false}>
      Navigation controls introduced in PR #2577. `back_button` is a strict boolean, defaulting to `false`. Set it to `true` to enable Back. See [Back navigation](/en/latest/humanize_navigation) for setup and hosted availability requirements.
    </ParamField>
...
<ParamField path="navigation" type="dict" required={false}>
  Navigation controls shared by supported humanize question schemas. `allow_back_past` is a strict boolean, defaulting to `true`. Set it to `false` to prevent moving back from this question to earlier items. Respondents can still return to and revise this question. This setting does not enable Back by itself. See [Back navigation](/en/latest/humanize_navigation).
</ParamField>
