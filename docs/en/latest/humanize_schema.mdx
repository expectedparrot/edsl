...
<ParamField path="exclusive_options" type="array" default={[]}>
  **Checkbox with other only** (`checkbox_with_other`). Options that stand alone: checking one clears every other selection — including the respondent's "other" entries and any other exclusive option — and selecting anything else clears it. Use for options like `"None of the above"` or `"Prefer not to say"`.

  Each entry is the exact text of an option in the question's `question_options` (surrounding whitespace is trimmed, and entries must be non-empty and unique). Because an exclusive option is an ordinary member of the option list, the submitted answer looks like any other selection. Defaults to an empty list, meaning no option is exclusive.
</ParamField>
