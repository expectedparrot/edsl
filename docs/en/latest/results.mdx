...
Pass a JSON-compatible `metadata` dictionary when constructing a `Result` to attach collection details or navigation history to an interview. Query these values with `results.select("metadata.*")`. See [Result metadata](/en/latest/result_metadata) for examples and serialization behavior introduced in PR #2577.
