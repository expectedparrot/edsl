...
async def test_initial_tokens():
    bucket = TokenBucket(bucket_name = "test", bucket_type = "requests", capacity=5, refill_rate=1)
...
async def test_get_multiple_tokens():
    ...
    assert bucket.tokens == 2, "Token count should decrease by the number of tokens requested"
