...
def test_results_example(db_mixin):
    from edsl.results import Results
    ...
    sql_output = r.sql("select * from self where id = 1 and data_type = 'model'", shape = "long", csv=True)
...
def test_wide_format():
    ...
    sql_results =  r.sql('select how_feeling from self', shape = "wide", remove_prefix = True, csv=True)
