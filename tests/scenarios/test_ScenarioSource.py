...
def test_all_source_types():
    ...
    failed_sources = [
        source_type for source_type, success in results.items() if not success
    ]
...
def test_each_source_type():
    ...
    for source_type in Source.get_registered_types():
        ...
        ), f"to_scenario_list did not return a ScenarioList for {source_type}"
...
def test_source_type_uniqueness():
    ...
    registered_types = Source.get_registered_types()
    assert len(registered_types) == len(
        set(registered_types)
    ), "Duplicate source types found in registry"
