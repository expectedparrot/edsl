...
import sqlite3
import pytest
from edsl.data.CacheHandler import CacheHandler, CacheEntry
...
def cache_handler():
    ...
    CacheHandler.OLD_CACHE = os.path.join(temp_dir, "edsl_cache.db")
...
def test_from_old_sqlite_cache_db(cache_handler, tmp_path):
    ...
    old_cache_path = tmp_path / "old_cache.db"
    ...
    with conn:
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, ("test_model", str({"key": "value"}), "test_system_prompt", "test_prompt", "test_output"))
    conn.close()

    # Call the from_old_sqlite_cache_db method
    newdata = cache_handler.from_old_sqlite_cache_db(old_cache_path)
