...
def test_initial_answers_basic():
    ...
    questions = [
        QuestionFreeText(question_name="user_name", question_text="What is your name?"),
        QuestionMultipleChoice(
            question_name="user_color",
            question_text="What is your favorite color?",
            question_options=["Red", "Blue", "Green", "Yellow"],
        ),
        QuestionNumerical(question_name="user_age", question_text="How old are you?"),
        QuestionFreeText(
            question_name="user_feedback", question_text="Any final comments?"
        ),
    ]
    ...
    print(
        f"✓ Widget without initial answers starts at: {widget1.current_question_name}"
    )
    ...
    initial_answers_2 = {"user_name": "Bob", "user_color": "Blue"}
...
def test_set_initial_answers():
    ...
    print(
        f"✓ Initial state: question={widget.current_question_name}, answers={widget.get_answers()}"
    )

    # Set initial answers using method
    widget.set_initial_answers({"q1": "Answer 1"})
    print(
        f"✓ After set_initial_answers: question={widget.current_question_name}, answers={widget.get_answers()}"
    )

    # Add another initial answer
    widget.add_initial_answer("q2", "B")
    print(
        f"✓ After add_initial_answer: question={widget.current_question_name}, answers={widget.get_answers()}"
    )
