...
class TestInferenceServiceRegistry:
    ...
    def mock_source_handler(self):
        ...
        handler.fetch_model_info_data.return_value = {
            "service1": [
                ModelInfo.from_raw({"id": "model1"}, "service1", "model1"),
                ModelInfo.from_raw({"id": "model2"}, "service1", "model2"),
            ],
            "service2": [
                ModelInfo.from_raw({"id": "model3"}, "service2", "model3"),
                ModelInfo.from_raw({"id": "model4"}, "service2", "model4"),
            ],
            "anthropic": [
                ModelInfo.from_raw(
                    {"id": "claude-3-opus"}, "anthropic", "claude-3-opus"
                ),
                ModelInfo.from_raw(
                    {"id": "claude-3-sonnet"}, "anthropic", "claude-3-sonnet"
                ),
            ],
            "openai": [
                ModelInfo.from_raw({"id": "gpt-4"}, "openai", "gpt-4"),
                ModelInfo.from_raw({"id": "gpt-3.5-turbo"}, "openai", "gpt-3.5-turbo"),
            ],
        }
    ...
    def test_model_info_data_property(self, mock_handler_class, registry):
        ...
        mock_handler.fetch_model_info_data.return_value = {
            "service1": [ModelInfo.from_raw({"id": "model1"}, "service1", "model1")]
        }
    ...
    def test_get_service_for_model_with_preferences(
        ...
        mock_source_handler.fetch_model_info_data.return_value = {
            "anthropic": [
                ModelInfo.from_raw({"id": "shared_model"}, "anthropic", "shared_model")
            ],
            "openai": [
                ModelInfo.from_raw({"id": "shared_model"}, "openai", "shared_model")
            ],
            "other": [
                ModelInfo.from_raw({"id": "shared_model"}, "other", "shared_model")
            ],
        }
    ...
    def test_get_service_for_model_fallback(self, registry, mock_source_handler):
        ...
        mock_source_handler.fetch_model_info_data.return_value = {
            "uncommon_service": [
                ModelInfo.from_raw(
                    {"id": "rare_model"}, "uncommon_service", "rare_model"
                )
            ]
        }
        registry._model_info_data = None  # Reset cache
    ...
    def test_get_services_for_model(self, registry, mock_source_handler):
        ...
        mock_source_handler.fetch_model_info_data.return_value = {
            "service1": [
                ModelInfo.from_raw({"id": "shared_model"}, "service1", "shared_model")
            ],
            "service2": [
                ModelInfo.from_raw({"id": "shared_model"}, "service2", "shared_model")
            ],
            "service3": [
                ModelInfo.from_raw({"id": "other_model"}, "service3", "other_model")
            ],
        }
    ...
    def test_create_language_model_with_service_name(self, registry):
        ...
        model = registry.create_language_model(
            "test_model", service_name="test_service"
        )
    ...
    def test_create_language_model_test_model(self, registry):
        ...
        with patch(
            "edsl.inference_services.services.test_service.TestService"
        ) as mock_test_service:
            mock_instance = Mock()
    ...
    def test_create_language_model_no_service_for_model(
        self, registry, mock_source_handler
    ):
    ...
    def test_source_preference_methods(self, registry):
        ...
        with patch.object(
            registry._source_handler, "get_source_preferences"
        ) as mock_get:
