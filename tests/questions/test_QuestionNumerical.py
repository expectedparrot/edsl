...
from edsl.questions import Question, QuestionNumerical, Settings
from edsl.questions.QuestionNumerical import QuestionNumericalEnhanced
...
def test_QuestionNumerical_construction():
    ...
    assert isinstance(q, QuestionNumericalEnhanced)
...
def test_QuestionNumerical_serialization():
    ...
    valid_question_w_type.update({"type": "numerical"})
    ...
    valid_question_wo_extras_w_type.update(
        {"type": "numerical", "min_value": None, "max_value": None}
    )
    ...
    assert isinstance(q_lazarus, QuestionNumericalEnhanced)
    assert type(q) == type(q_lazarus)
    assert repr(q) == repr(q_lazarus)

    # serialization from bad data should raise an exception
    with pytest.raises(Exception):
        Question.from_dict({"type": "numerical"})
    with pytest.raises(Exception):
        Question.from_dict({"type": "list", "question_text": 1})
    with pytest.raises(Exception):
        Question.from_dict({"type": "list", "question_text": ""})
    with pytest.raises(Exception):
        Question.from_dict({"type": "list", "question_text": "", "min_value": "0"})
    with pytest.raises(Exception):
        Question.from_dict({"type": "list", "question_text": "Asd", "min_value": "yes"})
    with pytest.raises(Exception):
        Question.from_dict(
            {"type": "list", "question_text": "Asd", "min_value": 4, "tires": 4}
        )
...
def test_test_QuestionNumerical_extras():
    ...
    # form elements
    assert "<label>How many planets are there?</label>" in q.form_elements()
